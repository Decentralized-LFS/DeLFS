...
TEST_SO="tests/checkpoint_example.so"
LOG_FILE="checkpoint_example_delfs.log"
...
echo "=================================================="
echo "Running test: checkpoint (delfs)"
...
RET=${PIPESTATUS[0]}
if [ "$RET" -ne 0 ]; then
    echo "ERROR: checkpoint (delfs) failed with exit code $RET"
    exit "$RET"
fi

echo "checkpoint (delfs) test completed successfully."
