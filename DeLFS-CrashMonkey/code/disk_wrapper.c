...
static int __init disk_wrapper_init(void) 
{
    ...
    target_device = blkdev_get_by_path(strim(target_device_path),
    ...
    flags_device = blkdev_get_by_path(strim(flags_device_path), 
...
out:
    ...
    if (Device.target_bd) {
        blkdev_put(Device.target_bd, &Device);
        Device.target_bd = NULL;
    }
