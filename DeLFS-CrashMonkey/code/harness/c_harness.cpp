...
static int LoadChangeData(fs_testing::Tester &test_harness) {
  const int change_fd = open(kChangePath, O_RDONLY | O_CLOEXEC);
  if (change_fd < 0) {
  ...
  const int ret = test_harness.GetChangeData(change_fd);
  close(change_fd);

  if (ret != SUCCESS) {
    std::cerr << "Error parsing change data" << std::endl;
    return -1;
  }
  ...
  if (log_file_load.empty()) {
    ...
    */
    ...
    if (background) {
	  ...
	  if (checkpoint == 0) {
	    change_fd = open(kChangePath,
			     O_CREAT | O_WRONLY | O_TRUNC | O_CLOEXEC,
			     S_IRUSR | S_IWUSR);
	  } else {
	    // checkpoint rerun에서는 change trace를 저장하지 않지만,
	    // Run(change_fd, checkpoint)가 write(change_fd, ...)를 해도
	    // EBADF가 나지 않도록 /dev/null을 넘긴다.
	    change_fd = open("/dev/null", O_WRONLY | O_CLOEXEC);
	  }
...
//          }
        ...
        if (checkpoint == 0) {
          ...
          if (LoadChangeData(test_harness) != SUCCESS) {
            test_harness.cleanup_harness();
            return -1;
          }
