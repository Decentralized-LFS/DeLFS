...
int Tester::GetChangeData(const int fd) {
  ...
  size_t nr_mods = 0;
  size_t nr_ckpt_mods = 0;

  while (true) {
    uint64_t buf;
    const int read_res = read(fd, (void *)&buf, sizeof(uint64_t));
    if (read_res < 0) {
      std::cerr << "GetChangeData: read(size) failed errno=" << errno << std::endl;
      return read_res;
    } else if (read_res == 0) {
      std::cerr << "GetChangeData: EOF before any/new DiskMod" << std::endl;
      break;
    }

    uint64_t next_chunk_size = be64toh(buf);
    std::cerr << "GetChangeData: next_chunk_size=" << next_chunk_size << std::endl;

    std::shared_ptr<char> data(new char[next_chunk_size], [](char *c) { delete[] c; });
    memcpy(data.get(), (void *)&buf, sizeof(uint64_t));

    unsigned long long read_data = sizeof(uint64_t);
    while (read_data < next_chunk_size) {
      const int res = read(fd, data.get() + read_data, next_chunk_size - read_data);
      if (res <= 0) {
        std::cerr << "GetChangeData: short read while reading body, read_data="
                  << read_data << " chunk=" << next_chunk_size
    ...
    if (res < 0) {
      std::cerr << "GetChangeData: DiskMod::Deserialize failed res=" << res << std::endl;
      return res;
    }

    if (mod.mod_type == DiskMod::kCheckpointMod) {
      nr_ckpt_mods++;
      mods_.push_back(std::vector<DiskMod>());
      std::cerr << "GetChangeData: checkpoint mod parsed, buckets=" << mods_.size() << std::endl;
    } else {
      nr_mods++;
      if (mods_.empty())
        mods_.push_back(std::vector<DiskMod>());
  ...
  std::cerr << "GetChangeData done: mods_.size()=" << mods_.size()
            << " nr_mods=" << nr_mods
            << " nr_ckpt_mods=" << nr_ckpt_mods
            << std::endl;
...
vector<milliseconds> Tester::test_fsck_and_user_test(
  ...
  const bool auto_check_ready =
      automate_check_test &&
      last_checkpoint > 0 &&
      checkpointToSnapshot_.find(last_checkpoint) != checkpointToSnapshot_.end() &&
      last_checkpoint < mods_.size() &&
      !mods_.at(last_checkpoint - 1).empty();

  if (automate_check_test && !auto_check_ready) {
    std::cout << "Skipping auto snapshot check at checkpoint "
              << last_checkpoint
              << " (no matching DiskMod bucket), fallback to check_test()"
              << std::endl;
  }

  if (auto_check_ready) {
    bool retVal = check_disk_and_snapshot_contents(device_path, last_checkpoint);
    if (!retVal) {
...
bool Tester::check_disk_and_snapshot_contents(std::string disk_path,
                                              int last_checkpoint) {
    if (checkpointToSnapshot_.find(last_checkpoint) == checkpointToSnapshot_.end()) {
        std::cout << "ERROR: no saved snapshot at checkpoint "
                  << last_checkpoint << std::endl;
        return false;
    }

    if (!(last_checkpoint > 0 &&
          last_checkpoint < static_cast<int>(mods_.size()))) {
        std::cerr << "ERROR: checkpoint/mod mismatch: "
                  << "last_checkpoint=" << last_checkpoint
                  << " mods_.size()=" << mods_.size()
                  << " checkpointToSnapshot_.size()="
                  << checkpointToSnapshot_.size()
                  << std::endl;
        return false;
    }

    std::string snapshot_path = checkpointToSnapshot_[last_checkpoint];
    std::ofstream diff_file;
    diff_file.open("diff-at-check" + std::to_string(last_checkpoint),
