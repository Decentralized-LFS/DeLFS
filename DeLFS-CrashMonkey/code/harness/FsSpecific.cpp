...
const char kDeLFSMkfsCommand[] =
    "/home/syslab/workspace_hwan/DeLFS/delfs-mkfs/mkfs/mkfs.delfs -f ";
const char kDeLFSFsckCommand[] =
    "/home/syslab/workspace_hwan/DeLFS/delfs-mkfs/fsck/fsck.delfs ";
}
...
FsSpecific* GetFsSpecific(std::string &fs_type) {
  ...
  } else if (fs_type.compare(DeLFSFsSpecific::kFsType) == 0) {
    return new DeLFSFsSpecific();
...
/******************************* DeLFS (DeLFS) ******************************************/
constexpr char DeLFSFsSpecific::kFsType[];

string DeLFSFsSpecific::GetMkfsCommand(string &device_path) {
  return string(kDeLFSMkfsCommand) + device_path;
}

string DeLFSFsSpecific::GetPostReplayMntOpts() {
  return string();
}

//string DeLFSFsSpecific::GetFsckCommand(const string &fs_path) {
//  return string(kDeLFSFsckCommand) + fs_path + " -y";
//}

/* Fsck not implemented for DeLfs */
string DeLFSFsSpecific::GetFsckCommand(const string &fs_path) {
	  return "/bin/true";
}

string DeLFSFsSpecific::GetNewUUIDCommand(const string &disk_path) {
  return string(kF2fsNewUUIDCommand);
}

FileSystemTestResult::ErrorType DeLFSFsSpecific::GetFsckReturn(
    int return_code) {
  if (return_code == 0) {
    return FileSystemTestResult::kFixed;
  }
...
string DeLFSFsSpecific::GetFsTypeString() {
  return string(DeLFSFsSpecific::kFsType);
}

unsigned int DeLFSFsSpecific::GetPostRunDelaySeconds() {
  return DeLFSFsSpecific::kDelaySeconds;
}
