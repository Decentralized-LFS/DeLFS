...
//				f3fs_up_write(&F3FS_I(inode)->i_gc_rwsem[WRITE]);
      ...
      gc_buf[off] = f3fs_get_read_data_page_without_cache(inode, start_bidx, REQ_RAHEAD, true, NULL);
      if (!gc_buf[off]) {
...
//			f3fs_up_write(&F3FS_I(inode)->i_gc_rwsem[WRITE]);
		...
		if (inode) {
			...
			if (S_ISREG(inode->i_mode)) {
				range_w = f3fs_down_write_range_trylock3(&fi->i_gc_rwsem[WRITE], start_bidx, 1);
			        if (!range_w) {
					sbi->skipped_gc_rwsem++;
					continue;
				}
  ...
  if (*victim == NULL_SEGNO) {
    if (multiple_victim) {
      ...
      if (*victim == NULL_SEGNO) {
        ...
        ret = get_multiple_victim_by_default(
          sbi,
          multiple_victim,
          gc_type,
          NO_CHECK_TYPE,
...
//	down_write(&sit_i->sentry_lock);
  down_write(&sit_i->last_victim_lock);
	ret = f3fs_get_victim(sbi, victim, gc_type, NO_CHECK_TYPE, LFS, 0);
...
int do_gc_internal(struct f3fs_sb_info *sbi, struct f3fs_gc_control *gc_control, char worker_idx, unsigned int* multiple_victim)
{
	int gc_type = gc_control->init_gc_type;
	unsigned int segno = gc_control->victim_segno;
	int sec_freed = 0, seg_freed = 0, total_freed = 0, total_sec_freed = 0;
...
int do_gc(struct f3fs_sb_info *sbi, struct f3fs_gc_control *gc_control, char worker_idx, unsigned int* multiple_victim)
{
	...
	ktime_t stopwatch[2];
	ktget(&stopwatch[0]);
	tmp = do_gc_internal(sbi, gc_control, worker_idx, multiple_victim);
	ktget(&stopwatch[1]);
	ktput(stopwatch, do_gc);
	return tmp;
}
