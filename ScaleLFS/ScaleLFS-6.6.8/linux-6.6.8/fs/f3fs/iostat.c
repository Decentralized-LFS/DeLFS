...
static inline unsigned long long iostat_get_avg_bytes(struct f3fs_sb_info *sbi,
...
static inline void __f3fs_update_iostat(struct f3fs_sb_info *sbi,
...
void f3fs_update_iostat(struct f3fs_sb_info *sbi, struct inode *inode,
			enum iostat_type type, unsigned long long io_bytes)
{
	unsigned long flags;
	...
	spin_lock_irqsave(&sbi->iostat_lock, flags);
	__f3fs_update_iostat(sbi, type, io_bytes);

	if (type == APP_BUFFERED_IO || type == APP_DIRECT_IO)
		__f3fs_update_iostat(sbi, APP_WRITE_IO, io_bytes);

	if (type == APP_BUFFERED_READ_IO || type == APP_DIRECT_READ_IO)
		__f3fs_update_iostat(sbi, APP_READ_IO, io_bytes);

#ifdef CONFIG_F3FS_FS_COMPRESSION
	if (inode && f3fs_compressed_file(inode)) {
		if (type == APP_BUFFERED_IO)
			__f3fs_update_iostat(sbi, APP_BUFFERED_CDATA_IO, io_bytes);

		if (type == APP_BUFFERED_READ_IO)
			__f3fs_update_iostat(sbi, APP_BUFFERED_CDATA_READ_IO, io_bytes);

		if (type == APP_MAPPED_READ_IO)
			__f3fs_update_iostat(sbi, APP_MAPPED_CDATA_READ_IO, io_bytes);

		if (type == APP_MAPPED_IO)
			__f3fs_update_iostat(sbi, APP_MAPPED_CDATA_IO, io_bytes);

		if (type == FS_DATA_READ_IO)
			__f3fs_update_iostat(sbi, FS_CDATA_READ_IO, io_bytes);

		if (type == FS_DATA_IO)
			__f3fs_update_iostat(sbi, FS_CDATA_IO, io_bytes);
	}
#endif
				...
				enum iostat_lat_type lat_type)
{
	...
	if (page_type == META_FLUSH) {
		...
		f3fs_warn(sbi, "%s: %d over NR_PAGE_TYPE", __func__, page_type);
		return;
	}
