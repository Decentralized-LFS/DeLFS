...
EXPORT_SYMBOL(do_reconfigure_mnt);
...
EXPORT_SYMBOL(do_move_mount_old);
...
long (*conn_do_mount)(const char *dev_name, const char __user *dir_name,
		const char *type_page, unsigned long flags, void *data_page);
EXPORT_SYMBOL(conn_do_mount);
		...
		const char *type_page, unsigned long flags, void *data_page)
{
	...
	char uptr_name[9];
	if (conn_do_mount) {
		strncpy_from_user(uptr_name, dir_name, 9);
		if (!strncmp(uptr_name, "/mnt/test", 9)) {
			return (*conn_do_mount)(dev_name, dir_name, type_page, flags, data_page);
		}
	}
