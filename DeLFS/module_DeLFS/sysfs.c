...
struct f3fs_attr {
	...
	ssize_t (*show)(struct f3fs_attr *a, struct f3fs_sb_info *sbi, char *buf);
	ssize_t (*store)(struct f3fs_attr *a, struct f3fs_sb_info *sbi,
...
static ssize_t cp_status_show(struct f3fs_attr *a,
		struct f3fs_sb_info *sbi, char *buf)
{
	return sysfs_emit(buf, "%x\n", le32_to_cpu(F3FS_CKPT(sbi)->ckpt_flags));
}
...
static ssize_t gc_mode_show(struct f3fs_attr *a,
		struct f3fs_sb_info *sbi, char *buf)
{
...
out:
	...
	if (a->struct_type == RESERVED_BLOCKS) {
		...
		if (t > (unsigned long)(atomic_read(&sbi->user_block_count) -
		...
		*ui = t;
		...
		int tmp = min(atomic_read(&sbi->reserved_blocks), 
				atomic_read(&sbi->user_block_count) - valid_user_blocks(sbi));
		atomic_set(&sbi->current_reserved_blocks, tmp);
		spin_unlock(&sbi->stat_lock);
		return count;
	}
	...
	if (!strcmp(a->attr.name, "ipu_policy")) {
		if (t >= BIT(F3FS_IPU_MAX))
			return -EINVAL;
		if (t && f3fs_lfs_mode(sbi))
			return -EINVAL;
...
#define STAT_INFO_RO_ATTR(name, elname)				\
	F3FS_RO_ATTR(STAT_INFO, f3fs_stat_info, name, elname)
#endif

#define GC_THREAD_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(GC_THREAD, f3fs_gc_kthread, name, elname)

#define SM_INFO_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(SM_INFO, f3fs_sm_info, name, elname)

#define SM_INFO_GENERAL_RW_ATTR(elname)				\
	SM_INFO_RW_ATTR(elname, elname)

#define DCC_INFO_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(DCC_INFO, discard_cmd_control, name, elname)

#define DCC_INFO_GENERAL_RW_ATTR(elname)			\
	DCC_INFO_RW_ATTR(elname, elname)

#define NM_INFO_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(NM_INFO, f3fs_nm_info, name, elname)

#define NM_INFO_GENERAL_RW_ATTR(elname)				\
	NM_INFO_RW_ATTR(elname, elname)

#define F3FS_SBI_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(F3FS_SBI, f3fs_sb_info, name, elname)

#define F3FS_SBI_GENERAL_RW_ATTR(elname)			\
	F3FS_SBI_RW_ATTR(elname, elname)

#define F3FS_SBI_GENERAL_RO_ATTR(elname)			\
	F3FS_RO_ATTR(F3FS_SBI, f3fs_sb_info, elname, elname)

#ifdef CONFIG_F2FS_FAULT_INJECTION
#define FAULT_INFO_GENERAL_RW_ATTR(type, elname)		\
	F3FS_RW_ATTR(type, f3fs_fault_info, elname, elname)
#endif

#define RESERVED_BLOCKS_GENERAL_RW_ATTR(elname)			\
	F3FS_RW_ATTR(RESERVED_BLOCKS, f3fs_sb_info, elname, elname)

#define CPRC_INFO_GENERAL_RW_ATTR(elname)			\
	F3FS_RW_ATTR(CPRC_INFO, ckpt_req_control, elname, elname)

#define ATGC_INFO_RW_ATTR(name, elname)				\
	F3FS_RW_ATTR(ATGC_INFO, atgc_management, name, elname)
...
F3FS_SBI_RW_ATTR(gc_idle, gc_mode);
F3FS_SBI_RW_ATTR(gc_urgent, gc_mode);
F3FS_SBI_RW_ATTR(cp_interval, interval_time[CP_TIME]);
F3FS_SBI_RW_ATTR(idle_interval, interval_time[REQ_TIME]);
F3FS_SBI_RW_ATTR(discard_idle_interval, interval_time[DISCARD_TIME]);
F3FS_SBI_RW_ATTR(gc_idle_interval, interval_time[GC_TIME]);
F3FS_SBI_RW_ATTR(umount_discard_timeout, interval_time[UMOUNT_DISCARD_TIMEOUT]);
F3FS_SBI_RW_ATTR(gc_pin_file_thresh, gc_pin_file_threshold);
F3FS_SBI_RW_ATTR(gc_reclaimed_segments, gc_reclaimed_segs);
F3FS_SBI_GENERAL_RW_ATTR(max_victim_search);
F3FS_SBI_GENERAL_RW_ATTR(migration_granularity);
F3FS_SBI_GENERAL_RW_ATTR(dir_level);
#ifdef CONFIG_F2FS_IOSTAT
F3FS_SBI_GENERAL_RW_ATTR(iostat_enable);
F3FS_SBI_GENERAL_RW_ATTR(iostat_period_ms);
#endif
F3FS_SBI_GENERAL_RW_ATTR(readdir_ra);
F3FS_SBI_GENERAL_RW_ATTR(max_io_bytes);
F3FS_SBI_GENERAL_RW_ATTR(data_io_flag);
F3FS_SBI_GENERAL_RW_ATTR(node_io_flag);
F3FS_SBI_GENERAL_RW_ATTR(gc_remaining_trials);
F3FS_SBI_GENERAL_RW_ATTR(seq_file_ra_mul);
F3FS_SBI_GENERAL_RW_ATTR(gc_segment_mode);
F3FS_SBI_GENERAL_RW_ATTR(max_fragment_chunk);
F3FS_SBI_GENERAL_RW_ATTR(max_fragment_hole);
#ifdef CONFIG_F2FS_FS_COMPRESSION
F3FS_SBI_GENERAL_RW_ATTR(compr_written_block);
F3FS_SBI_GENERAL_RW_ATTR(compr_saved_block);
F3FS_SBI_GENERAL_RW_ATTR(compr_new_inode);
F3FS_SBI_GENERAL_RW_ATTR(compress_percent);
F3FS_SBI_GENERAL_RW_ATTR(compress_watermark);
#endif
/* atomic write */
F3FS_SBI_GENERAL_RO_ATTR(current_atomic_write);
F3FS_SBI_GENERAL_RW_ATTR(peak_atomic_write);
F3FS_SBI_GENERAL_RW_ATTR(committed_atomic_block);
F3FS_SBI_GENERAL_RW_ATTR(revoked_atomic_block);
/* block age extent cache */
F3FS_SBI_GENERAL_RW_ATTR(hot_data_age_threshold);
F3FS_SBI_GENERAL_RW_ATTR(warm_data_age_threshold);
F3FS_SBI_GENERAL_RW_ATTR(last_age_weight);
#ifdef CONFIG_BLK_DEV_ZONED
F3FS_SBI_GENERAL_RO_ATTR(unusable_blocks_per_sec);
#endif
...
F3FS_GENERAL_RO_ATTR(cp_status);
...
static const struct kobj_type f3fs_sb_ktype = {
	.default_groups = f3fs_groups,
	.sysfs_ops	= &f3fs_attr_ops,
	.release	= f3fs_sb_release,
};

static const struct kobj_type f3fs_ktype = {
	.sysfs_ops	= &f3fs_attr_ops,
};
...
static const struct kobj_type f3fs_feat_ktype = {
...
static const struct kobj_type f3fs_stat_ktype = {
...
static const struct kobj_type f3fs_feature_list_ktype = {
...
{
	remove_proc_subtree(sbi->sb->s_id, f3fs_proc_root);
