...
static void update_general_status(struct f3fs_sb_info *sbi)
{
	...
	si->total_count = (int)atomic_read(&sbi->user_block_count) / sbi->blocks_per_seg;
	...
	si->nats = atomic_read(&NM_I(sbi)->nat_cnt[TOTAL_NAT]);
	//si->dirty_nats = NM_I(sbi)->nat_cnt[DIRTY_NAT];
	si->dirty_nats = atomic_read(&NM_I(sbi)->nat_cnt[DIRTY_NAT]);
	si->sits = MAIN_SEGS(sbi);
	si->dirty_sits = atomic_read(&SIT_I(sbi)->dirty_sentries);
	//si->free_nids = NM_I(sbi)->nid_cnt[FREE_NID];
	si->free_nids = atomic_read(&NM_I(sbi)->nid_cnt[FREE_NID]);
	//si->avail_nids = NM_I(sbi)->available_nids;
	si->avail_nids = atomic_read(&NM_I(sbi)->available_nids);
	//si->alloc_nids = NM_I(sbi)->nid_cnt[PREALLOC_NID];
	si->alloc_nids = atomic_read(&NM_I(sbi)->nid_cnt[PREALLOC_NID]);
	si->io_skip_bggc = sbi->io_skip_bggc;
	si->other_skip_bggc = sbi->other_skip_bggc;
	si->util_free = (int)(free_user_blocks(sbi) >> sbi->log_blocks_per_seg)
		//* 100 / (int)(sbi->user_block_count >> sbi->log_blocks_per_seg)
		* 100 / (int)(atomic_read(&sbi->user_block_count) >> sbi->log_blocks_per_seg)
		/ 2;
...
get_cache:
	...
	/* free nids */
	si->cache_mem += (atomic_read(&NM_I(sbi)->nid_cnt[FREE_NID]) +
				atomic_read(&NM_I(sbi)->nid_cnt[PREALLOC_NID])) *
				sizeof(struct free_nid);
	si->cache_mem += atomic_read(&NM_I(sbi)->nat_cnt[TOTAL_NAT]) *
				sizeof(struct nat_entry);
	si->cache_mem += atomic_read(&NM_I(sbi)->nat_cnt[DIRTY_NAT]) *
...
static const char *ipu_mode_names[F3FS_IPU_MAX] = {
	[F3FS_IPU_FORCE]	= "FORCE",
	[F3FS_IPU_SSR]		= "SSR",
	[F3FS_IPU_UTIL]		= "UTIL",
	[F3FS_IPU_SSR_UTIL]	= "SSR_UTIL",
	[F3FS_IPU_FSYNC]	= "FSYNC",
	[F3FS_IPU_ASYNC]	= "ASYNC",
	[F3FS_IPU_NOCACHE]	= "NOCACHE",
	[F3FS_IPU_HONOR_OPU_WRITE]	= "HONOR_OPU_WRITE",
};
...
static int stat_show(struct seq_file *s, void *v)
{
	...
	list_for_each_entry(si, &f3fs_stat_list, stat_list) {
		struct f3fs_sb_info *sbi = si->sbi;

		update_general_status(sbi);

		seq_printf(s, "\n=====[ partition info(%pg). #%d, %s, CP: %s]=====\n",
			sbi->sb->s_bdev, i++,
			f3fs_readonly(sbi->sb) ? "RO" : "RW",
			is_set_ckpt_flags(sbi, CP_DISABLED_FLAG) ?
			"Disabled" : (f3fs_cp_error(sbi) ? "Error" : "Good"));
		if (sbi->s_flag) {
			seq_puts(s, "[SBI:");
		...
		seq_puts(s, "Policy:\n");
		seq_puts(s, "  - IPU: [");
		if (IS_F3FS_IPU_DISABLE(sbi)) {
			seq_puts(s, " DISABLE");
		} else {
			unsigned long policy = SM_I(sbi)->ipu_policy;

			for_each_set_bit(j, &policy, F3FS_IPU_MAX)
