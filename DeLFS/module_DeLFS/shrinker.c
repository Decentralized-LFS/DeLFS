...
static unsigned long __count_nat_entries(struct f3fs_sb_info *sbi)
{
	...
	return atomic_read(&NM_I(sbi)->nat_cnt[RECLAIMABLE_NAT]);
}

static unsigned long __count_free_nids(struct f3fs_sb_info *sbi)
{
	//long count = NM_I(sbi)->nid_cnt[FREE_NID] - MAX_FREE_NIDS;
	long count = atomic_read(&NM_I(sbi)->nid_cnt[FREE_NID]) - MAX_FREE_NIDS;
				...
				struct shrink_control *sc)
{
	...
	while (p != &f3fs_list) {
		...
		freed += f3fs_shrink_age_extent_tree(sbi, nr >> 2);

		/* shrink read extent cache entries */
		freed += f3fs_shrink_read_extent_tree(sbi, nr >> 2);

		/* shrink clean nat cache entries */
		if (freed < nr) {
			for_each_online_cpu(nat_cpu)
				freed += f3fs_try_to_free_nats(sbi, nr - freed, nat_cpu);
		}
...
{
	f3fs_shrink_read_extent_tree(sbi, __count_extent_cache(sbi, EX_READ));
