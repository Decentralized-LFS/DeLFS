...
bool f3fs_space_for_roll_forward(struct f3fs_sb_info *sbi)
{
	...
	if (atomic_read(&sbi->last_valid_block_count) + nalloc > atomic_read(&sbi->user_block_count))
		return false;
...
static int sanity_check_node_chain(struct f3fs_sb_info *sbi, block_t blkaddr,
		block_t *blkaddr_fast, bool *is_detecting)
{
	...
	for (i = 0; i < 2; i++) {
		if (!f3fs_is_valid_blkaddr(sbi, *blkaddr_fast, META_POR)) {
			*is_detecting = false;
			return 0;
		}

		page = f3fs_get_tmp_page(sbi, *blkaddr_fast);
		if (IS_ERR(page))
			return PTR_ERR(page);
		...
		*blkaddr_fast = next_blkaddr_of_node(page);
		...
		f3fs_ra_meta_pages_cond(sbi, *blkaddr_fast, ra_blocks);
	}

	if (*blkaddr_fast == blkaddr) {
		f3fs_notice(sbi, "%s: Detect looped node chain on blkaddr:%u."
				...
				bool check_only)
{
	...
	/* get node pages in the current segment */
		...
		curseg = cpu_array->array + CURSEG_WARM_NODE;
                        ...
                        block_t blkaddr, struct dnode_of_data *dn)
{
        ...
        /* Get the previous summary from any CPU's current DATA curseg first */
        for_each_online_cpu(cpu) {
...
got_it:
        ...
        if (ofs_in_node >= max_addrs) {
                ...
                f3fs_handle_error(sbi, ERROR_INCONSISTENT_SUMMARY);
                return -EFSCORRUPTED;
        }
...
//	max_addrs = ADDRS_PER_PAGE(dn->node_page, dn->inode);
//	if (ofs_in_node >= max_addrs) {
//		f3fs_err(sbi, "Inconsistent ofs_in_node:%u in summary, ino:%lu, nid:%u, max:%u",
//			ofs_in_node, dn->inode->i_ino, nid, max_addrs);
//		f3fs_handle_error(sbi, ERROR_INCONSISTENT_SUMMARY);
//		return -EFSCORRUPTED;
//	}
...
retry_prev:
			...
			if (f3fs_is_valid_blkaddr(sbi, dest,
					DATA_GENERIC_ENHANCE_UPDATE)) {
				f3fs_err(sbi, "Inconsistent dest blkaddr:%u, ino:%lu, ofs:%u",
                ...
                struct list_head *tmp_inode_list, struct list_head *dir_list)
{
        ...
        for_each_online_cpu(cpu) {
                struct curseg_info_array *cpu_array = WRAP_CURSEG_I(sbi, cpu);
                struct curseg_info *curseg = cpu_array->array + CURSEG_WARM_NODE;
