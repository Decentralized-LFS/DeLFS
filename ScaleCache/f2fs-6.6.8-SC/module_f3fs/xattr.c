...
static const struct xattr_handler *f3fs_xattr_handler_map[] = {
	...
	[F3FS_XATTR_INDEX_POSIX_ACL_ACCESS] = &nop_posix_acl_access,
	[F3FS_XATTR_INDEX_POSIX_ACL_DEFAULT] = &nop_posix_acl_default,
#endif
...
static inline const char *f3fs_xattr_prefix(int index,
				...
				bool *is_inline)
{
	...
	*xe = __find_xattr(cur_addr, last_txattr_addr, NULL, index, len, name);
	if (!*xe) {
		f3fs_err(F3FS_I_SB(inode), "lookup inode (%lu) has corrupted xattr",
								inode->i_ino);
...
ssize_t f3fs_listxattr(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	list_for_each_xattr(entry, base_addr) {
		...
		prefix = f3fs_xattr_prefix(entry->e_name_index, dentry);

		if ((void *)(entry) + sizeof(__u32) > last_base_addr ||
			(void *)XATTR_NEXT_ENTRY(entry) > last_base_addr) {
			f3fs_err(F3FS_I_SB(inode), "list inode (%lu) has corrupted xattr",
						inode->i_ino);
			set_sbi_flag(F3FS_I_SB(inode), SBI_NEED_FSCK);
			f3fs_handle_error(F3FS_I_SB(inode),
						ERROR_CORRUPTED_XATTR);
...
retry:
	...
	if (!here) {
		if (!F3FS_I(inode)->i_xattr_nid) {
			f3fs_notice(F3FS_I_SB(inode),
				"recover xattr in inode (%lu)", inode->i_ino);
			f3fs_recover_xattr_data(inode, NULL);
			kfree(base_addr);
			goto retry;
		}
		f3fs_err(F3FS_I_SB(inode), "set inode (%lu) has corrupted xattr",
								inode->i_ino);
		set_sbi_flag(F3FS_I_SB(inode), SBI_NEED_FSCK);
		error = -EFSCORRUPTED;
		f3fs_handle_error(F3FS_I_SB(inode),
