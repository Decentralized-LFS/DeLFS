 ...
 */
KTDEF(f2fs_stop_checkpoint);
KTDEF(f2fs_balance_fs_bg);
KTDEF(f2fs_balance_fs_if2);
KTDEF(f2fs_balance_fs_if3);
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	if (time_to_inject(sbi, FAULT_CHECKPOINT)) {
		...
		ktget(&stopwatch[1]);
		ktput(stopwatch, f2fs_stop_checkpoint);
	}
	...
	ktget(&stopwatch[1]);
	ktput(stopwatch, f2fs_balance_fs_bg);
	...
	if (test_opt(sbi, GC_MERGE) && sbi->gc_thread &&
		...
		ktget(&stopwatch[1]);
		ktput(stopwatch, f2fs_balance_fs_if2);
	} else {
		ktget(&stopwatch[0]);
		...
		ktget(&stopwatch[1]);
		ktput(stopwatch, f2fs_balance_fs_if3);
	}
}
...
// close original checkpoint at file operations' critical path
	...
	if (test_opt(sbi, DATA_FLUSH) && from_bg) {
		...
		ktget(&stopwatch[1]);
		ktput(stopwatch, f2fs_sync_dirty_inodes);
