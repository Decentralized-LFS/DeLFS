...
struct delfs_configuration c;

#ifdef HAVE_SPARSE_SPARSE_H
#include <sparse/sparse.h>
struct sparse_file *delfs_sparse_file;
static char **blocks;
...
static int sparse_merge_blocks(uint64_t start, uint64_t num, int zero)
{
	...
	if (zero) {
		blocks[start] = NULL;
		return sparse_file_add_fill(delfs_sparse_file, 0x0,
	...
	return sparse_file_add_data(delfs_sparse_file, blocks[start],
					F2FS_BLKSIZE * num, start);
}
#else
...
/*
 * Copied from fs/delfs/segment.c
 */
/*
 ...
 */
enum rw_hint delfs_io_type_to_rw_hint(int seg_type)
{
	switch (seg_type) {
...
int cp_dev_write_block(void *buf, __u64 blk_addr, enum rw_hint whint)
{
	//return dev_write(buf, blk_addr << F2FS_BLKSIZE_BITS, F2FS_BLKSIZE, whint);
	return dev_write(buf, blk_addr << F2FS_BLKSIZE_BITS, F2FS_BLKSIZE * 4, whint);
}
...
int delfs_init_sparse_file(void)
{
#ifdef HAVE_SPARSE_SPARSE_H
	if (c.func == MKFS) {
		delfs_sparse_file = sparse_file_new(F2FS_BLKSIZE, c.device_size);
		if (!delfs_sparse_file)
			return -1;
	} else {
		delfs_sparse_file = sparse_file_import(c.devices[0].fd,
							true, false);
		if (!delfs_sparse_file)
			return -1;

		c.blksize = sparse_file_block_size(delfs_sparse_file);
		...
		c.device_size = sparse_file_len(delfs_sparse_file, 0, 0);
	...
	return sparse_file_foreach_chunk(delfs_sparse_file, true, false,
...
void delfs_release_sparse_resource(void)
{
#ifdef HAVE_SPARSE_SPARSE_H
	int j;

	if (c.sparse_mode) {
		if (delfs_sparse_file != NULL) {
			sparse_file_destroy(delfs_sparse_file);
			delfs_sparse_file = NULL;
		}
...
int delfs_finalize_device(void)
{
	int i;
	int ret = 0;
	...
	if (c.sparse_mode) {
		...
		if (c.func != MKFS) {
			...
			delfs_sparse_file = sparse_file_new(F2FS_BLKSIZE,
							c.device_size);
		}
		...
		sparse_file_write(delfs_sparse_file, c.devices[0].fd,
