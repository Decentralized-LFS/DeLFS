...
int delfs_test_bit(unsigned int nr, const char *p)
{
	int mask;
	char *addr = (char *)p;
...
int delfs_set_bit(unsigned int nr, char *addr)
{
	int mask;
	int ret;
...
int delfs_clear_bit(unsigned int nr, char *addr)
{
	int mask;
	int ret;
 ...
 */
static delfs_hash_t __delfs_dentry_hash(const unsigned char *name, int len)/* Need update */
{
	__u32 hash;
	delfs_hash_t	delfs_hash;
	...
	delfs_hash = cpu_to_le32(hash & ~F2FS_HASH_COL_BIT);
	return delfs_hash;
}

delfs_hash_t delfs_dentry_hash(int encoding, int casefolded,
                             const unsigned char *name, int len)
{
	const struct delfs_nls_table *table = delfs_load_nls_table(encoding);
	int r, dlen;
	unsigned char *buff;
	...
	if (len && casefolded) {
		...
		r = __delfs_dentry_hash(buff, dlen);

		free(buff);
		return r;
	}
opaque_seq:
	return __delfs_dentry_hash(name, len);
}

unsigned int addrs_per_page(struct delfs_inode *i, bool is_inode)
{
...
unsigned int delfs_max_file_offset(struct delfs_inode *i)
{
	if (!LINUX_S_ISREG(le16_to_cpu(i->i_mode)) ||
			!(le32_to_cpu(i->i_flags) & F2FS_COMPR_FL))
...
uint32_t delfs_cal_crc32(uint32_t crc, void *buf, int len)
{
	int i;
...
int delfs_crc_valid(uint32_t blk_crc, void *buf, int len)
{
	uint32_t cal_crc = 0;

	cal_crc = delfs_cal_crc32(F2FS_SUPER_MAGIC, buf, len);
...
__u32 delfs_inode_chksum(struct delfs_node *node)
{
	struct delfs_inode *ri = &node->i;
	__le32 ino = F2FS_NODE_FOOTER(node)->ino;
	__le32 gen = ri->i_generation;
	__u32 chksum, chksum_seed;
	__u32 dummy_cs = 0;
	unsigned int offset = offsetof(struct delfs_inode, i_inode_checksum);
	unsigned int cs_size = sizeof(dummy_cs);

	chksum = delfs_cal_crc32(c.chksum_seed, (__u8 *)&ino,
							sizeof(ino));
	chksum_seed = delfs_cal_crc32(chksum, (__u8 *)&gen, sizeof(gen));

	chksum = delfs_cal_crc32(chksum_seed, (__u8 *)ri, offset);
	chksum = delfs_cal_crc32(chksum, (__u8 *)&dummy_cs, cs_size);
	offset += cs_size;
	chksum = delfs_cal_crc32(chksum, (__u8 *)ri + offset,
						F2FS_BLKSIZE - offset);
	return chksum;
}

__u32 delfs_checkpoint_chksum(struct delfs_checkpoint *cp)
{
	unsigned int chksum_ofs = le32_to_cpu(cp->checksum_offset);
	__u32 chksum;

	chksum = delfs_cal_crc32(F2FS_SUPER_MAGIC, cp, chksum_ofs);
	if (chksum_ofs < CP_CHKSUM_OFFSET) {
		chksum_ofs += sizeof(chksum);
		chksum = delfs_cal_crc32(chksum, (__u8 *)cp + chksum_ofs,
						//(F2FS_BLKSIZE - chksum_ofs);
						(F2FS_BLKSIZE*4) - chksum_ofs);
	}
	return chksum;
}

int write_inode(struct delfs_node *inode, u64 blkaddr, enum rw_hint whint)
{
	if (c.feature & F2FS_FEATURE_INODE_CHKSUM) {
		inode->i.i_inode_checksum =
			cpu_to_le32(delfs_inode_chksum(inode));
	}
 ...
 * device information
 */
void delfs_init_configuration(void)
{
	int i, j;

	memset(&c, 0, sizeof(struct delfs_configuration));
	c.ndevs = 1;
	...
	c.cur_segs = malloc(num_cpus * sizeof(uint32_t *));
	if (!c.cur_segs) {
		perror("malloc cur_seg failed");
		exit(EXIT_FAILURE);
	}

	for (i = 0; i < num_cpus; i++) {
		c.cur_segs[i] = malloc(NR_CURSEG_TYPE * sizeof(uint32_t));
		if (!c.cur_segs[i]) {
			perror("malloc cur_seg failed");
			while (--i >= 0) free(c.cur_segs[i]);
			free(c.cur_segs);
			exit(EXIT_FAILURE);
		}
	...
	c.curseg_offsets = malloc(num_cpus * sizeof(block_t *));
	if (!c.curseg_offsets) {
		perror("malloc curseg_offsets failed");
		free(c.cur_segs);
		exit(EXIT_FAILURE);
	}

	for (i = 0; i < num_cpus; i++) {
		c.curseg_offsets[i] = malloc(NR_CURSEG_TYPE * sizeof(block_t));
		if (!c.curseg_offsets[i]) {
			perror("malloc cur_seg failed");
			while (--i >= 0) free(c.curseg_offsets[i]);
			free(c.curseg_offsets);
			while (--num_cpus >= 0) free(c.cur_segs[i]);
			free(c.cur_segs);
			exit(EXIT_FAILURE);
		}
		for (j = 0; j < NR_CURSEG_TYPE; j++) {
			c.curseg_offsets[i][j] = 0;
		}
	}
...
int delfs_dev_is_writable(void)
{
	return !c.ro || c.force;
}
...
int delfs_dev_is_umounted(char *path)
{
#ifdef _WIN32
	return 0;
#else
	 ...
	 * delfs_stop_checkpoint makes RO in /proc/mounts while RW in /etc/mtab.
	 */
#ifdef __linux__
...
#define MOUNTED _PATH_MOUNTED
	...
	/*
	 * If delfs is umounted with -l, the process can still use
...
int delfs_devs_are_umounted(void)
{
	int ret, i;

	for (i = 0; i < c.ndevs; i++) {
		ret = delfs_dev_is_umounted((char *)c.devices[i].path);
		if (ret)
			return ret;
	}
	return 0;
...
int get_device_info(int i)
{
	...
	if (c.sparse_mode && i==0 && delfs_init_sparse_file())
		return -1;
	return 0;
}
#endif

int delfs_get_device_info(void)
{
	int i;
...
{
	unsigned int size = offsetof(struct delfs_inode, i_projid);

	if (c.feature & F2FS_FEATURE_FLEXIBLE_INLINE_XATTR)
		size = offsetof(struct delfs_inode, i_projid);
	if (c.feature & F2FS_FEATURE_PRJQUOTA)
		size = offsetof(struct delfs_inode, i_inode_checksum);
	if (c.feature & F2FS_FEATURE_INODE_CHKSUM)
		size = offsetof(struct delfs_inode, i_crtime);
	if (c.feature & F2FS_FEATURE_INODE_CRTIME)
		size = offsetof(struct delfs_inode, i_compr_blocks);
	if (c.feature & F2FS_FEATURE_COMPRESSION)
		size = offsetof(struct delfs_inode, i_extra_end);
...
int delfs_str2encoding(const char *string)
{
	int i;

	for (i = 0 ; i < ARRAY_SIZE(delfs_encoding_map); i++)
		if (!strcmp(string, delfs_encoding_map[i].name))
			return delfs_encoding_map[i].encoding_magic;

	return -EINVAL;
}

char *delfs_encoding2str(const int encoding)
{
	int i;

	for (i = 0 ; i < ARRAY_SIZE(delfs_encoding_map); i++)
		if (delfs_encoding_map[i].encoding_magic == encoding)
			return delfs_encoding_map[i].name;

	return NULL;
}

int delfs_get_encoding_flags(int encoding)
{
	int i;

	for (i = 0 ; i < ARRAY_SIZE(delfs_encoding_map); i++)
		if (delfs_encoding_map[i].encoding_magic == encoding)
			return delfs_encoding_map[encoding].default_flags;

	return 0;
}

int delfs_str2encoding_flags(char **param, __u16 *flags)
{
	char *f = strtok(*param, ",");
