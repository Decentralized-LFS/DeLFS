...
int delfs_get_zoned_model(int i)
{
...
uint32_t delfs_get_zone_chunk_sectors(struct device_info *dev)
{
	uint32_t sectors;
	char str[PATH_MAX];
	FILE *file;
...
int delfs_get_zone_blocks(int i)
{
	struct device_info *dev = c.devices + i;
	uint64_t sectors;
	...
	sectors = delfs_get_zone_chunk_sectors(dev);
	if (!sectors)
		return -1;
...
int delfs_report_zone(int i, uint64_t sector, struct blk_zone *blkzone)
{
...
int delfs_report_zones(int j, report_zones_cb_t *report_zones_cb, void *opaque)
{
...
int delfs_reset_zone(int i, void *blkzone)
{
...
uint32_t delfs_get_usable_segments(struct delfs_super_block *sb)
{
...
int delfs_report_zone(int i, uint64_t UNUSED(sector),
...
int delfs_report_zones(int i, report_zones_cb_t *UNUSED(report_zones_cb),
					void *UNUSED(opaque))
{
	ERR_MSG("%d: Unsupported zoned block device\n", i);
	return -1;
...
int delfs_check_zones(int i)
{
	ERR_MSG("%d: Unsupported zoned block device\n", i);
	return -1;
}

int delfs_reset_zone(int i, void *UNUSED(blkzone))
{
	ERR_MSG("%d: Unsupported zoned block device\n", i);
	return -1;
}

int delfs_reset_zones(int i)
{
	ERR_MSG("%d: Unsupported zoned block device\n", i);
	return -1;
}

int delfs_finish_zone(int i, void *UNUSED(blkzone))
{
	ERR_MSG("%d: Unsupported zoned block device\n", i);
	return -1;
}
