...
static int migrate_block(struct delfs_sb_info *sbi, u64 from, u64 to)
{
	...
	/* write to */
	ret = dev_write_block(raw, to, delfs_io_type_to_rw_hint(se->type));
	ASSERT(ret >= 0);
...
int delfs_defragment(struct delfs_sb_info *sbi, u64 from, u64 len, u64 to, int left)
{
	struct seg_entry *se;
	u64 idx, offset;
	...
	for (idx = from; idx < from + len; idx++) {
		...
		if (!delfs_test_bit(offset, (const char *)se->cur_valid_map))
			continue;
