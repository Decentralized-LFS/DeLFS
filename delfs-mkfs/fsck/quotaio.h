 ...
 *
 * Hyojun Kim <hyojun@google.com> - Ported to delfs-tools
 */
 ...
 * filesystem. */
errcode_t quota_file_open(struct delfs_sb_info *sbi, struct quota_handle *h,
			  enum quota_type qtype, int flags);

/* Create new quotafile of specified format on given filesystem */
errcode_t quota_file_create(struct delfs_sb_info *sbi, struct quota_handle *h,
		enum quota_type qtype);

/* Close quotafile */
errcode_t quota_file_close(struct delfs_sb_info *sbi, struct quota_handle *h,
		int update_filesize);
...
/* In mkquota.c */
errcode_t quota_init_context(struct delfs_sb_info *sbi);
void quota_data_inodes(quota_ctx_t qctx, struct delfs_inode *inode, int adjust);
void quota_data_add(quota_ctx_t qctx, struct delfs_inode *inode, qsize_t space);
void quota_data_sub(quota_ctx_t qctx, struct delfs_inode *inode, qsize_t space);
errcode_t quota_write_inode(struct delfs_sb_info *sbi, enum quota_type qtype);
void quota_add_inode_usage(quota_ctx_t qctx, delfs_ino_t ino,
		struct delfs_inode* inode);
void quota_release_context(quota_ctx_t *qctx);
errcode_t quota_compare_and_update(struct delfs_sb_info *sbi,
