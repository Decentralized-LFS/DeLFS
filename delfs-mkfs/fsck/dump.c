...
void nat_dump(struct delfs_sb_info *sbi, nid_t start_nat, nid_t end_nat)
{
	struct delfs_nat_block *nat_block;
	struct delfs_node *node_block;
	...
	ASSERT(nat_block);
	node_block = (struct delfs_node *)calloc(F2FS_BLKSIZE, 1);
	ASSERT(node_block);
...
void sit_dump(struct delfs_sb_info *sbi, unsigned int start_sit,
					unsigned int end_sit)
{
	...
	for (segno = start_sit; segno < end_sit; segno++) {
		...
		"\nsegno:%8u\tvblocks:%3u\tseg_type:%d\tsit_pack:%d\n\n",
			...
			delfs_test_bit(offset, sit_i->sit_bitmap) ? 2 : 1);
...
void ssa_dump(struct delfs_sb_info *sbi, int start_ssa, int end_ssa)
{
	...
	snprintf(buf, BUF_SZ, "Note: dump.delfs -b blkaddr = 0x%x + segno * "
				" 0x200 + offset\n",
...
static void dump_folder_contents(struct delfs_sb_info *sbi, u8 *bitmap,
...
static void dump_data_blk(struct delfs_sb_info *sbi, __u64 offset, u32 blkaddr, int type)
{
	...
	/* get data */
	if (blkaddr == NEW_ADDR ||
			!delfs_is_valid_blkaddr(sbi, blkaddr, DATA_GENERIC)) {
	...
	if (S_ISDIR(type)) {
		struct delfs_dentry_block *d = (struct delfs_dentry_block *) buf;
...
static void dump_node_blk(struct delfs_sb_info *sbi, int ntype,
...
static void dump_xattr(struct delfs_sb_info *sbi, struct delfs_node *node_blk, int type)
{
	void *xattr;
	void *last_base_addr;
...
#else
static void dump_xattr(struct delfs_sb_info *UNUSED(sbi),
				struct delfs_node *UNUSED(node_blk), int UNUSED(is_dir))
{
	MSG(0, "XATTR does not support\n");
}
#endif

static int dump_inode_blk(struct delfs_sb_info *sbi, u32 nid,
					struct delfs_node *node_blk)
{
	u32 i = 0;
	u64 ofs = 0;
	...
	c.show_file_map_max_offset = delfs_max_file_offset(&node_blk->i);
...
static void dump_file(struct delfs_sb_info *sbi, struct node_info *ni,
				struct delfs_node *node_blk, char *path)
{
	struct delfs_inode *inode = &node_blk->i;
	int ret;
...
static void dump_link(struct delfs_sb_info *sbi, struct node_info *ni,
				struct delfs_node *node_blk, char *name)
{
#if defined(__MINGW32__)
...
static void dump_folder(struct delfs_sb_info *sbi, struct node_info *ni,
				struct delfs_node *node_blk, char *path, int is_root)
{
	if (!is_root) {
#if defined(__MINGW32__)
...
static int dump_filesystem(struct delfs_sb_info *sbi, struct node_info *ni,
				struct delfs_node *node_blk, int force, char *base_path,
...
bool is_sit_bitmap_set(struct delfs_sb_info *sbi, u32 blk_addr)
{
	struct seg_entry *se;
	u32 offset;
	...
	return delfs_test_bit(offset,
			(const char *)se->cur_valid_map) != 0;
}

void dump_node_scan_disk(struct delfs_sb_info *sbi, nid_t nid)
{
	struct delfs_node *node_blk;
	pgoff_t blkaddr;
	int ret;
...
int dump_node(struct delfs_sb_info *sbi, nid_t nid, int force, char *base_path, int base, int allow_folder, char *dirent_name)
{
	struct node_info ni;
	struct delfs_node *node_blk;
	int ret = 0;
...
static void dump_node_from_blkaddr(struct delfs_sb_info *sbi, u32 blk_addr)
{
	struct delfs_node *node_blk;
	int ret;
...
static void dump_dirent(u32 blk_addr, int is_inline, int enc_name)
{
	...
	if (is_inline) {
		inline_dentry = inline_data_addr((struct delfs_node *)blk);
...
int dump_info_from_blkaddr(struct delfs_sb_info *sbi, u32 blk_addr)
{
	nid_t nid;
	int type;
