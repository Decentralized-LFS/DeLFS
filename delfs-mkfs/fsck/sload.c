...
static int delfs_make_directory(struct delfs_sb_info *sbi,
...
#ifdef HAVE_LIBSELINUX
static int set_selinux_xattr(struct delfs_sb_info *sbi, const char *path,
							nid_t ino, int mode)
{
	char *secontext = NULL;
	char *mnt_path = NULL;
...
static int build_directory(struct delfs_sb_info *sbi, const char *full_path,
			...
			nid_t dir_ino)
{
	...
	ret = delfs_make_directory(sbi, entries, dentries);
	if (ret)
		goto out_free;

	for (i = 0; i < entries; i++) {
		if (dentries[i].file_type == F2FS_FT_REG_FILE) {
			delfs_build_file(sbi, dentries + i);
			...
			/*
			 * It is already done in delfs_make_directory
			 * delfs_make_symlink(sbi, dir_ino, &dentries[i]);
			 */
		} else {
...
int delfs_sload(struct delfs_sb_info *sbi)
{
	int ret = 0;
