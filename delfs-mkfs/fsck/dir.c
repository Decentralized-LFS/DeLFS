 ...
 * dir.c
 *
 * Many parts of codes are copied from Linux kernel/fs/delfs.
 *
...
void make_dentry_ptr(struct delfs_dentry_ptr *d, struct delfs_node *node_blk,
							void *src, int type)
{
	if (type == 1) {
		struct delfs_dentry_block *t = (struct delfs_dentry_block *)src;
		...
		d->bitmap = (u8 *)src;
		d->dentry = (struct delfs_dir_entry *)
...
static struct delfs_dir_entry *find_target_dentry(const u8 *name,
		unsigned int len, delfs_hash_t namehash, int *max_slots,
...
static struct delfs_dir_entry *find_in_block(void *block,
		const u8 *name, int len, delfs_hash_t namehash,
		int *max_slots)
{
	struct delfs_dentry_ptr d;
...
static int find_in_level(struct delfs_sb_info *sbi, struct delfs_node *dir,
		unsigned int level, struct dentry *de)
{
	unsigned int nbucket, nblock;
	unsigned int bidx, end_block;
	struct delfs_dir_entry *dentry = NULL;
	struct dnode_of_data dn;
	void *dentry_blk;
	int max_slots = 214;
	nid_t ino = le32_to_cpu(F2FS_NODE_FOOTER(dir)->ino);
	delfs_hash_t namehash;
	unsigned int dir_level = dir->i.i_dir_level;
	int ret = 0;

	namehash = delfs_dentry_hash(get_encoding(sbi), IS_CASEFOLDED(&dir->i),
					de->name, de->len);
...
static int delfs_find_entry(struct delfs_sb_info *sbi,
				struct delfs_node *dir, struct dentry *de)
{
...
nid_t delfs_lookup(struct delfs_sb_info *sbi, struct delfs_node *dir,
				u8 *name, int len)
{
	int err;
	struct dentry de = {
	...
	err = delfs_find_entry(sbi, dir, &de);
	if (err == 1)
		return de.ino;
	else
		return 0;
...
static void delfs_update_dentry(nid_t ino, int file_type,
		struct delfs_dentry_ptr *d,
		const unsigned char *name, int len, delfs_hash_t name_hash,
...
/*
 * delfs_add_link - Add a new file(dir) to parent dir.
 */
int delfs_add_link(struct delfs_sb_info *sbi, struct delfs_node *parent,
			...
			int file_type, block_t *p_blkaddr, int inc_link)
{
	...
	delfs_hash_t dentry_hash;
	struct delfs_dentry_block *dentry_blk;
	struct delfs_dentry_ptr d;
	struct dnode_of_data dn;
	nid_t pino;
	...
	dentry_hash = delfs_dentry_hash(get_encoding(sbi),
...
add_dentry:
	...
	delfs_update_dentry(ino, file_type, &d, name, name_len, dentry_hash, bit_pos);
...
static void make_empty_dir(struct delfs_sb_info *sbi, struct delfs_node *inode)
{
...
static void page_symlink(struct delfs_sb_info *sbi, struct delfs_node *inode,
					const char *symname, int symlen)
{
	...
	ret = dev_write_block(data_blk, blkaddr,
			      delfs_io_type_to_rw_hint(CURSEG_WARM_DATA));
	ASSERT(ret >= 0);
...
static void set_file_temperature(struct delfs_sb_info *sbi,
...
static void init_inode_block(struct delfs_sb_info *sbi,
		struct delfs_node *node_blk, struct dentry *de)
{
	...
	if (c.feature & F2FS_FEATURE_INODE_CHKSUM)
		node_blk->i.i_inode_checksum =
			cpu_to_le32(delfs_inode_chksum(node_blk));
}
							...
							block_t *p_blkaddr)
{
	...
	if (!dir_level) {
		struct delfs_dentry_block *dentry_blk;
		struct delfs_dentry_ptr src, dst;
	...
	while (bit_pos < (unsigned long)d.max) {
		...
		ret = delfs_add_link(sbi, node, filename, namelen,
				le32_to_cpu(de->ino),
...
int delfs_create(struct delfs_sb_info *sbi, struct dentry *de)
{
	struct delfs_node *parent, *child;
	struct hardlink_cache_entry *found_hardlink = NULL;
	...
	if (de->from_devino)
		found_hardlink = delfs_search_hardlink(sbi, de);
	...
	ret = delfs_find_entry(sbi, parent, de);
	if (ret) {
	...
	if (found_hardlink && found_hardlink->to_ino) {
		...
		delfs_alloc_nid(sbi, &de->ino);
	}

	init_inode_block(sbi, child, de);

	ret = delfs_add_link(sbi, parent, child->i.i_name,
...
write_child_dir:
	ret = nodeblk_alloced ? dev_write_block(child, blkaddr,
			delfs_io_type_to_rw_hint(CURSEG_HOT_NODE)) :
...
int delfs_mkdir(struct delfs_sb_info *sbi, struct dentry *de)
{
	return delfs_create(sbi, de);
}

int delfs_symlink(struct delfs_sb_info *sbi, struct dentry *de)
{
	return delfs_create(sbi, de);
}

int delfs_find_path(struct delfs_sb_info *sbi, char *path, nid_t *ino)
{
	struct delfs_node *parent;
	struct node_info ni;
	struct dentry de;
	int err = 0;
	int ret;
