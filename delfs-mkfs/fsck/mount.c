...
static int get_device_idx(struct delfs_sb_info *sbi, uint32_t segno)
{
...
static int get_zone_idx_from_dev(struct delfs_sb_info *sbi,
...
bool is_usable_seg(struct delfs_sb_info *sbi, unsigned int segno)
{
...
unsigned int get_usable_seg_count(struct delfs_sb_info *sbi)
{
...
bool is_usable_seg(struct delfs_sb_info *UNUSED(sbi), unsigned int UNUSED(segno))
{
	return true;
}
...
u32 get_free_segments(struct delfs_sb_info *sbi)
{
	u32 i, free_segs = 0;
...
void update_free_segments(struct delfs_sb_info *sbi)
{
	char *progress = "-*|*-";
	static int i = 0;
...
{
	const struct delfs_acl_header *hdr = (struct delfs_acl_header *)value;
	const struct delfs_acl_entry *entry = (struct delfs_acl_entry *)(hdr + 1);
	...
	for (i = 0; i < count; i++) {
		...
		case ACL_OTHER:
			...
			entry = (struct delfs_acl_entry *)((char *)entry +
					sizeof(struct delfs_acl_entry_short));
			break;
		case ACL_USER:
...
static void print_xattr_entry(const struct delfs_xattr_entry *ent)
{
...
void print_inode_info(struct delfs_sb_info *sbi,
			struct delfs_node *node, int name)
{
	struct delfs_inode *inode = &node->i;
	void *xattr_addr;
	void *last_base_addr;
	struct delfs_xattr_entry *ent;
...
void print_node_info(struct delfs_sb_info *sbi,
			struct delfs_node *node_block, int verbose)
{
...
void print_extention_list(struct delfs_super_block *sb, int cold)
{
	int start, end, i;
...
void print_sb_debug_info(struct delfs_super_block *sb);
void print_raw_sb_info(struct delfs_super_block *sb)
{
#ifdef HAVE_LIBUUID
	char uuid[40];
...
void print_ckpt_info(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
printout:
	...
	DISP_u32(cp, cur_nodes_segno[0][0]);
	DISP_u32(cp, cur_nodes_segno[0][1]);
	DISP_u32(cp, cur_nodes_segno[0][2]);

	DISP_u32(cp, cur_nodes_blkoff[0][0]);
	DISP_u32(cp, cur_nodes_blkoff[0][1]);
	DISP_u32(cp, cur_nodes_blkoff[0][2]);


	DISP_u32(cp, alloc_type[CURSEG_HOT_DATA]);
	DISP_u32(cp, alloc_type[CURSEG_WARM_DATA]);
	DISP_u32(cp, alloc_type[CURSEG_COLD_DATA]);
	DISP_u32(cp, cur_datas_segno[0][0]);
	DISP_u32(cp, cur_datas_segno[0][1]);
	DISP_u32(cp, cur_datas_segno[0][2]);

	DISP_u32(cp, cur_datas_blkoff[0][0]);
	DISP_u32(cp, cur_datas_blkoff[0][1]);
	DISP_u32(cp, cur_datas_blkoff[0][2]);
...
void print_sb_state(struct delfs_super_block *sb)
{
...
void print_sb_stop_reason(struct delfs_super_block *sb)
{
...
void print_sb_errors(struct delfs_super_block *sb)
{
...
bool delfs_is_valid_blkaddr(struct delfs_sb_info *sbi,
...
static inline block_t current_sit_addr(struct delfs_sb_info *sbi,
						unsigned int start);

/*
 * Readahead CP/NAT/SIT/SSA pages
 */
int delfs_ra_meta_pages(struct delfs_sb_info *sbi, block_t start, int nrpages,
							int type)
{
	block_t blkno = start;
	...
	for (; nrpages-- > 0; blkno++) {
		...
		if (!delfs_is_valid_blkaddr(sbi, blkno, type))
			goto out;
...
void update_superblock(struct delfs_super_block *sb, int sb_mask)
{
	int addr, ret;
	uint8_t *buf;
	u32 old_crc, new_crc;
	...
	if (get_sb(feature) & F2FS_FEATURE_SB_CHKSUM) {
		old_crc = get_sb(crc);
		new_crc = delfs_cal_crc32(F2FS_SUPER_MAGIC, sb,
						SB_CHKSUM_OFFSET);
		set_sb(crc, new_crc);
...
static inline int sanity_check_area_boundary(struct delfs_super_block *sb,
...
static int verify_sb_chksum(struct delfs_super_block *sb)
{
	...
	if (delfs_crc_valid(get_sb(crc), sb,
			get_sb(checksum_offset))) {
...
int sanity_check_raw_super(struct delfs_super_block *sb, enum SB_ADDR sb_addr)
{
...
int validate_super_block(struct delfs_sb_info *sbi, enum SB_ADDR sb_addr)
{
	char buf[F2FS_BLKSIZE];

	sbi->raw_super = malloc(sizeof(struct delfs_super_block));
	if (!sbi->raw_super)
		return -ENOMEM;
	...
	memcpy(sbi->raw_super, buf + F2FS_SUPER_OFFSET,
					sizeof(struct delfs_super_block));
...
int init_sb_info(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	u64 total_sectors;
	int i;
...
static int verify_checksum_chksum(struct delfs_checkpoint *cp)
{
	...
	cal_crc = delfs_checkpoint_chksum(cp);
	if (cal_crc != crc) {
...
static void *get_checkpoint_version(block_t cp_addr)
{
	...
	if (verify_checksum_chksum((struct delfs_checkpoint *)cp_page))
		goto out;
	return cp_page;
out:
	free(cp_page);
...
void *validate_checkpoint(struct delfs_sb_info *sbi, block_t cp_addr,
				unsigned long long *version)
{
	void *cp_page_1, *cp_page_2;
	struct delfs_checkpoint *cp;
	...
	cp = (struct delfs_checkpoint *)cp_page_1;
	...
	cp = (struct delfs_checkpoint *)cp_page_2;
...
int get_valid_checkpoint(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
bool is_checkpoint_stop(struct delfs_super_block *sb, bool abnormal)
{
	int i;
...
bool is_inconsistent_error(struct delfs_super_block *sb)
{
	int i;
 ...
 */
static int delfs_should_proceed(struct delfs_super_block *sb, u32 flag)
{
...
int sanity_check_ckpt(struct delfs_sb_info *sbi)
{
	...
	if (!(get_sb(feature) & F2FS_FEATURE_RO) &&
		(fsmeta < F2FS_MIN_SEGMENT || ovp_segments == 0 ||
					reserved_segments == 0)) {
		MSG(0, "\tWrong layout: check mkfs.delfs version\n");
		return 1;
	}
	...
	if (!user_block_count || user_block_count >=
		...
		if (!delfs_should_proceed(sb, flag))
			return 1;
		if (!c.fix_on)
			return 1;
	...
	for (i = 0; i < NR_CURSEG_NODE_TYPE; i++) {
		if (get_cp(cur_nodes_segno[0][i]) >= main_segs ||
			get_cp(cur_nodes_blkoff[0][i]) >= blocks_per_seg)
			return 1;
	}
	for (i = 0; i < NR_CURSEG_DATA_TYPE; i++) {
		if (get_cp(cur_datas_segno[0][i]) >= main_segs ||
			get_cp(cur_datas_blkoff[0][i]) >= blocks_per_seg)
			return 1;
	}
...
pgoff_t current_nat_addr(struct delfs_sb_info *sbi, nid_t start, int *pack)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	pgoff_t block_off;
	pgoff_t block_addr;
	int seg_off;
	...
	if (delfs_test_bit(block_off, nm_i->nat_bitmap)) {
...
static int delfs_early_init_nid_bitmap(struct delfs_sb_info *sbi)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	int nid_bitmap_size = (nm_i->max_nid + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_summary_block *sum = curseg->sum_blk;
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(sum);
	nid_t nid;
	int i;
	...
	delfs_set_bit(0, nm_i->nid_bitmap);

	if (nats_in_cursum(journal) > NAT_JOURNAL_ENTRIES) {
		MSG(0, "\tError: delfs_init_nid_bitmap truncate n_nats(%u) to "
	...
	for (i = 0; i < nats_in_cursum(journal); i++) {
		...
		if (addr != NULL_ADDR &&
			!delfs_is_valid_blkaddr(sbi, addr, DATA_GENERIC)) {
			MSG(0, "\tError: delfs_init_nid_bitmap: addr(%u) is invalid!!!\n", addr);
		...
		if (!IS_VALID_NID(sbi, nid)) {
			MSG(0, "\tError: delfs_init_nid_bitmap: nid(%u) is invalid!!!\n", nid);
			journal->n_nats = cpu_to_le16(i);
			c.fix_on = 1;
			continue;
		}
		if (addr != NULL_ADDR)
			delfs_set_bit(nid, nm_i->nid_bitmap);
	}
	return 0;
}

/* will init nid_bitmap from nat */
static int delfs_late_init_nid_bitmap(struct delfs_sb_info *sbi)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	struct delfs_nat_block *nat_block;
	block_t start_blk;
	nid_t nid;
	...
	delfs_ra_meta_pages(sbi, 0, NAT_BLOCK_OFFSET(nm_i->max_nid),
							META_NAT);
...
u32 update_nat_bits_flags(struct delfs_super_block *sb,
				struct delfs_checkpoint *cp, u32 flags)
{
...
void write_nat_bits(struct delfs_sb_info *sbi,
	struct delfs_super_block *sb, struct delfs_checkpoint *cp, int set)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	...
	for (i = 0; i < nat_blocks; i++) {
		 ...
		 * old one.
		 */
		if (i < nm_i->nat_blocks && delfs_test_bit(i, nm_i->nat_bitmap))
...
static int check_nat_bits(struct delfs_sb_info *sbi,
	struct delfs_super_block *sb, struct delfs_checkpoint *cp)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	uint32_t nat_blocks = get_sb(segment_count_nat) <<
				(get_sb(log_blocks_per_seg) - 1);
	...
	unsigned char *nat_bits, *full_nat_bits, *empty_nat_bits;
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	uint32_t i, j;
	block_t blkaddr;
	int err = 0;
	...
	if (*(__le64 *)nat_bits != get_cp_crc(cp) || nats_in_cursum(journal)) {
		/*
		 * if there is a journal, delfs was not shutdown cleanly. Let's
	...
	for (i = 0; i < nat_blocks; i++) {
		...
		for (j = 0; j < NAT_ENTRY_PER_BLOCK; j++) {
			if (delfs_test_bit(start_nid + j, nm_i->nid_bitmap))
				valid++;
		}
		if (valid == 0) {
			if (!empty || full) {
...
int init_node_manager(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	struct delfs_nm_info *nm_i = NM_I(sbi);
	...
	return delfs_early_init_nid_bitmap(sbi);
}

int build_node_manager(struct delfs_sb_info *sbi)
{
	int err;
	sbi->nm_info = malloc(sizeof(struct delfs_nm_info));
	if (!sbi->nm_info)
		return -ENOMEM;
...
int build_sit_info(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
void reset_curseg(struct delfs_sb_info *sbi, int type)
{
...
static void read_compacted_summaries(struct delfs_sb_info *sbi)
{
	...
	for (i = CURSEG_HOT_DATA; i <= CURSEG_COLD_DATA; i++) {
		...
		for (j = 0; j < blk_off; j++) {
			struct delfs_summary *s;
			s = (struct delfs_summary *)(kaddr + offset);
...
static void restore_node_summary(struct delfs_sb_info *sbi,
		unsigned int segno, struct delfs_summary_block *sum_blk)
{
	struct delfs_node *node_blk;
	struct delfs_summary *sum_entry;
	block_t addr;
	unsigned int i;
	int ret;
...
static void read_normal_summaries(struct delfs_sb_info *sbi, int type)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	struct delfs_summary_block *sum_blk;
	struct curseg_info *curseg;
	...
	if (IS_DATASEG(type)) {
		segno = get_cp(cur_datas_segno[0][type]);
		if (is_set_ckpt_flags(cp, CP_UMOUNT_FLAG))
			blk_addr = sum_blk_addr(sbi, NR_CURSEG_TYPE, type);
		else
			blk_addr = sum_blk_addr(sbi, NR_CURSEG_DATA_TYPE, type);
	} else {
		segno = get_cp(cur_nodes_segno[0][type - CURSEG_HOT_NODE]);
		if (is_set_ckpt_flags(cp, CP_UMOUNT_FLAG))
...
void update_sum_entry(struct delfs_sb_info *sbi, block_t blk_addr,
					struct delfs_summary *sum)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_summary_block *sum_blk;
	u32 segno, offset;
...
static void restore_curseg_summaries(struct delfs_sb_info *sbi)
{
...
static int build_curseg(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	...
	for (i = 0; i < NR_CURSEG_TYPE; i++) {
		...
		if (i <= CURSEG_COLD_DATA) {
			blk_off = get_cp(cur_datas_blkoff[0][i]);
			segno = get_cp(cur_datas_segno[0][i]);
		}
		if (i > CURSEG_COLD_DATA) {
			blk_off = get_cp(cur_nodes_blkoff[0][i - CURSEG_HOT_NODE]);
			segno = get_cp(cur_nodes_segno[0][i - CURSEG_HOT_NODE]);
		}
...
static inline void check_seg_range(struct delfs_sb_info *sbi, unsigned int segno)
{
						...
						unsigned int segno)
{
	...
	if (delfs_test_bit(offset, sit_i->sit_bitmap))
		blk_addr += sit_i->sit_blocks;

	return blk_addr;
}

void get_current_sit_page(struct delfs_sb_info *sbi,
			unsigned int segno, struct delfs_sit_block *sit_blk)
{
...
void rewrite_current_sit_page(struct delfs_sb_info *sbi,
			unsigned int segno, struct delfs_sit_block *sit_blk)
{
	block_t blk_addr = current_sit_addr(sbi, segno);
...
void check_block_count(struct delfs_sb_info *sbi,
		unsigned int segno, struct delfs_sit_entry *raw_sit)
{
	struct delfs_sm_info *sm_info = SM_I(sbi);
...
void __seg_info_from_raw_sit(struct seg_entry *se,
		struct delfs_sit_entry *raw_sit)
{
...
void seg_info_from_raw_sit(struct delfs_sb_info *sbi, struct seg_entry *se,
						struct delfs_sit_entry *raw_sit)
{
...
struct seg_entry *get_seg_entry(struct delfs_sb_info *sbi,
		unsigned int segno)
{
	struct sit_info *sit_i = SIT_I(sbi);
...
unsigned short get_seg_vblocks(struct delfs_sb_info *sbi, struct seg_entry *se)
{
	if (!need_fsync_data_record(sbi))
...
unsigned char *get_seg_bitmap(struct delfs_sb_info *sbi, struct seg_entry *se)
{
	if (!need_fsync_data_record(sbi))
...
unsigned char get_seg_type(struct delfs_sb_info *sbi, struct seg_entry *se)
{
	if (!need_fsync_data_record(sbi))
		return se->type;
	else
...
struct delfs_summary_block *get_sum_block(struct delfs_sb_info *sbi,
...
int get_sum_entry(struct delfs_sb_info *sbi, u32 blk_addr,
				struct delfs_summary *sum_entry)
{
	struct delfs_summary_block *sum_blk;
	u32 segno, offset;
...
static void get_nat_entry(struct delfs_sb_info *sbi, nid_t nid,
				struct delfs_nat_entry *raw_nat)
{
	struct delfs_nat_block *nat_block;
	pgoff_t block_addr;
	int entry_off;
	...
	nat_block = (struct delfs_nat_block *)calloc(F2FS_BLKSIZE, 1);
	ASSERT(nat_block);
	...
	memcpy(raw_nat, &nat_block->entries[entry_off],
					sizeof(struct delfs_nat_entry));
	free(nat_block);
}

void update_data_blkaddr(struct delfs_sb_info *sbi, nid_t nid,
		u16 ofs_in_node, block_t newaddr, struct delfs_node *node_blk)
{
	struct node_info ni;
	...
	if (node_blk == NULL) {
		node_blk = (struct delfs_node *)calloc(F2FS_BLKSIZE, 1);
		ASSERT(node_blk);
...
void update_nat_blkaddr(struct delfs_sb_info *sbi, nid_t ino,
					nid_t nid, block_t newaddr)
{
	struct delfs_nat_block *nat_block = NULL;
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct delfs_nat_entry *entry;
	pgoff_t block_addr;
	int entry_off;
	int ret, i;
...
void get_node_info(struct delfs_sb_info *sbi, nid_t nid, struct node_info *ni)
{
	struct delfs_nat_entry raw_nat;
...
static int build_sit_entries(struct delfs_sb_info *sbi)
{
	struct sit_info *sit_i = SIT_I(sbi);
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct delfs_sit_block *sit_blk;
	struct seg_entry *se;
	struct delfs_sit_entry sit;
	...
	do {
		readed = delfs_ra_meta_pages(sbi, start_blk, MAX_RA_BLOCKS,
								META_SIT);
...
static int early_build_segment_manager(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	struct delfs_sm_info *sm_info;

	sm_info = malloc(sizeof(struct delfs_sm_info));
	if (!sm_info) {
...
static int late_build_segment_manager(struct delfs_sb_info *sbi)
{
...
void build_sit_area_bitmap(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_sm_info *sm_i = SM_I(sbi);
	unsigned int segno = 0;
	char *ptr = NULL;
	u32 sum_vblocks = 0;
	u32 free_segs = 0;
...
void rewrite_sit_area_bitmap(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct sit_info *sit_i = SIT_I(sbi);
...
int flush_sit_journal_entries(struct delfs_sb_info *sbi)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct sit_info *sit_i = SIT_I(sbi);
...
int flush_nat_journal_entries(struct delfs_sb_info *sbi)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct delfs_nat_block *nat_block;
...
void flush_journal_entries(struct delfs_sb_info *sbi)
{
...
void flush_sit_entries(struct delfs_sb_info *sbi)
{
	struct sit_info *sit_i = SIT_I(sbi);
	struct delfs_sit_block *sit_blk;
	unsigned int segno = 0;
...
int relocate_curseg_offset(struct delfs_sb_info *sbi, int type)
{
	struct curseg_info *curseg = CURSEG_I(sbi, type);
	...
	for (i = 0; i < sbi->blocks_per_seg; i++) {
		if (!delfs_test_bit(i, (const char *)se->cur_valid_map))
			break;
	}
...
void set_section_type(struct delfs_sb_info *sbi, unsigned int segno, int type)
{
	unsigned int i;
...
static bool write_pointer_at_zone_start(struct delfs_sb_info *sbi,
					unsigned int zone_segno)
{
	...
	ret = delfs_report_zone(j, sector, &blkz);
	if (ret)
		return false;
...
static bool write_pointer_at_zone_start(struct delfs_sb_info *UNUSED(sbi),
...
static void zero_journal_entries_with_type(struct delfs_sb_info *sbi, int type)
{
	struct delfs_journal *journal =
...
int find_next_free_block(struct delfs_sb_info *sbi, u64 *to, int left,
...
next_segment:
		...
		if (type != want_type)
			goto next_segment;
				...
				!delfs_test_bit(offset, (const char *)bitmap))
			return 0;
...
void move_one_curseg_info(struct delfs_sb_info *sbi, u64 from, int left,
				 int i)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct curseg_info *curseg = CURSEG_I(sbi, i);
...
void move_curseg_info(struct delfs_sb_info *sbi, u64 from, int left)
{
	int i;
...
void update_curseg_info(struct delfs_sb_info *sbi, int type)
{
...
void zero_journal_entries(struct delfs_sb_info *sbi)
{
	int i;
...
void write_curseg_info(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	int i;
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		if (i < CURSEG_HOT_NODE) {
			set_cp(cur_datas_segno[0][i], CURSEG_I(sbi, i)->segno);
			set_cp(cur_datas_blkoff[0][i],
					CURSEG_I(sbi, i)->next_blkoff);
		} else {
			int n = i - CURSEG_HOT_NODE;

			set_cp(cur_nodes_segno[0][n], CURSEG_I(sbi, i)->segno);
			set_cp(cur_nodes_blkoff[0][n],
					CURSEG_I(sbi, i)->next_blkoff);
		}
	}
}

void save_curseg_warm_node_info(struct delfs_sb_info *sbi)
{
...
void restore_curseg_warm_node_info(struct delfs_sb_info *sbi)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_WARM_NODE);
	struct curseg_info *saved_curseg = &SM_I(sbi)->saved_curseg_warm_node;
...
int lookup_nat_in_journal(struct delfs_sb_info *sbi, u32 nid,
					struct delfs_nat_entry *raw_nat)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
...
void nullify_nat_entry(struct delfs_sb_info *sbi, u32 nid)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct delfs_nat_block *nat_block;
...
void duplicate_checkpoint(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
void write_checkpoint(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	...
	crc = delfs_checkpoint_chksum(cp);
...
void write_checkpoints(struct delfs_sb_info *sbi)
{
...
void write_raw_cp_blocks(struct delfs_sb_info *sbi,
			 struct delfs_checkpoint *cp, int which)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	uint32_t crc;
	block_t cp_blkaddr;
...
void build_nat_area_bitmap(struct delfs_sb_info *sbi)
{
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct delfs_journal *journal = F2FS_SUMMARY_BLOCK_JOURNAL(curseg->sum_blk);
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	fsck->entries = calloc(sizeof(struct delfs_nat_entry),
					fsck->nr_nat_entries);
	ASSERT(fsck->entries);
		...
		for (i = 0; i < NAT_ENTRY_PER_BLOCK; i++) {
			...
			if ((nid + i) == F2FS_NODE_INO(sbi) ||
				...
				if (le32_to_cpu(nat_block->entries[i].block_addr) != 0x1) {
					...
					delfs_set_bit(nid + i, fsck->nat_area_bitmap);
				}
				continue;
			}
	...
	for (i = 0; i < nats_in_cursum(journal); i++) {
		...
		if (fsck->entries[nid].block_addr != 0x0) {
			fsck->chk.valid_nat_entry_cnt--;
			delfs_clear_bit(nid, fsck->nat_area_bitmap);
		...
		if (ni.blk_addr != 0x0) {
			...
			delfs_set_bit(nid, fsck->nat_area_bitmap);
			fsck->chk.valid_nat_entry_cnt++;
...
static int check_sector_size(struct delfs_super_block *sb)
{
...
static int tune_sb_features(struct delfs_sb_info *sbi)
{
	int sb_changed = 0;
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
static int loop_node_chain_fix(block_t blkaddr_fast,
		struct delfs_node *node_blk_fast,
		block_t blkaddr, struct delfs_node *node_blk)
{
	...
	whint = delfs_io_type_to_rw_hint(CURSEG_WARM_NODE);
	if (IS_INODE(node_blk))
...
static int sanity_check_node_chain(struct delfs_sb_info *sbi,
		block_t *blkaddr_fast, struct delfs_node *node_blk_fast,
		block_t blkaddr, struct delfs_node *node_blk,
		bool *is_detecting)
{
	int i, err;
	...
	for (i = 0; i < 2; i++) {
		if (!delfs_is_valid_blkaddr(sbi, *blkaddr_fast, META_POR)) {
			*is_detecting = false;
			return 0;
		}
...
static int find_fsync_inode(struct delfs_sb_info *sbi, struct list_head *head)
{
	struct curseg_info *curseg;
	struct delfs_node *node_blk, *node_blk_fast;
	...
	while (1) {
		...
		if (!delfs_is_valid_blkaddr(sbi, blkaddr, META_POR))
			break;
...
static int do_record_fsync_data(struct delfs_sb_info *sbi,
					struct delfs_node *node_blk,
					block_t blkaddr)
{
	...
	if (delfs_test_bit(offset, (char *)se->cur_valid_map))
		return 1;

	if (delfs_test_bit(offset, (char *)se->ckpt_valid_map))
		return 1;
	...
	delfs_set_bit(offset, (char *)se->ckpt_valid_map);
...
static int traverse_dnodes(struct delfs_sb_info *sbi,
...
static int record_fsync_data(struct delfs_sb_info *sbi)
{
...
int delfs_do_mount(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = NULL;
	struct delfs_super_block *sb = NULL;
...
out:
	...
	if (c.feature & F2FS_FEATURE_INODE_CHKSUM)
		c.chksum_seed = delfs_cal_crc32(~0, sb->uuid, sizeof(sb->uuid));
	...
	if (!delfs_should_proceed(sb, get_cp(ckpt_flags)))
		return 1;
	...
	if (delfs_late_init_nid_bitmap(sbi)) {
		ERR_MSG("delfs_late_init_nid_bitmap failed\n");
		return -1;
	}
...
void delfs_do_umount(struct delfs_sb_info *sbi)
{
	struct sit_info *sit_i = SIT_I(sbi);
	struct delfs_sm_info *sm_i = SM_I(sbi);
	struct delfs_nm_info *nm_i = NM_I(sbi);
...
#ifdef WITH_ANDROID
int delfs_sparse_initialize_meta(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = sbi->raw_super;
...
#else
int delfs_sparse_initialize_meta(struct delfs_sb_info *sbi) { return 0; }
#endif
