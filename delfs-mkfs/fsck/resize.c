...
static int get_new_sb(struct delfs_super_block *sb)
{
...
static void migrate_main(struct delfs_sb_info *sbi, unsigned int offset)
{
...
static void move_ssa(struct delfs_sb_info *sbi, unsigned int segno,
...
static void migrate_ssa(struct delfs_sb_info *sbi,
		struct delfs_super_block *new_sb, unsigned int offset)
{
...
static int shrink_nats(struct delfs_sb_info *sbi,
				struct delfs_super_block *new_sb)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
static void migrate_nat(struct delfs_sb_info *sbi,
			struct delfs_super_block *new_sb)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_nm_info *nm_i = NM_I(sbi);
	...
	for (nid = nm_i->max_nid - 1; nid >= 0; nid -= NAT_ENTRY_PER_BLOCK) {
		...
		if (delfs_test_bit(block_off, nm_i->nat_bitmap)) {
			block_addr += sbi->blocks_per_seg;
			delfs_clear_bit(block_off, nm_i->nat_bitmap);
		}
...
static void migrate_sit(struct delfs_sb_info *sbi,
		struct delfs_super_block *new_sb, unsigned int offset)
{
			...
			struct delfs_super_block *new_sb, unsigned int offset)
{
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		if (i < CURSEG_HOT_NODE) {
			set_cp(cur_datas_segno[0][i],
					CURSEG_I(sbi, i)->segno - offset);
		} else {
			int n = i - CURSEG_HOT_NODE;

			set_cp(cur_nodes_segno[0][n],
					CURSEG_I(sbi, i)->segno - offset);
		}
	}
	...
	crc = delfs_checkpoint_chksum(new_cp);
...
static int delfs_resize_check(struct delfs_sb_info *sbi, struct delfs_super_block *new_sb)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
static int delfs_resize_grow(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_super_block new_sb_raw;
	struct delfs_super_block *new_sb = &new_sb_raw;
	...
	if (delfs_resize_check(sbi, new_sb) < 0)
		return -1;
	...
	if (new_main_blkaddr < end_blkaddr) {
		err = delfs_defragment(sbi, old_main_blkaddr, offset,
						new_main_blkaddr, 0);
		if (!err)
...
static int delfs_resize_shrink(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_super_block new_sb_raw;
	struct delfs_super_block *new_sb = &new_sb_raw;
	...
	err = delfs_defragment(sbi, tmp_end_blkaddr,
...
int delfs_resize(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	...
	if ((c.target_sectors * c.sector_size >>
		...
		if (!c.safe_resize) {
			...
			return delfs_resize_shrink(sbi);
		}
	else if (((c.target_sectors * c.sector_size >>
			get_sb(log_blocksize)) > get_sb(block_count)) ||
			c.force)
		return delfs_resize_grow(sbi);
	else {
