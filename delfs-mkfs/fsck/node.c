...
void delfs_alloc_nid(struct delfs_sb_info *sbi, nid_t *nid)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	nid_t i;

	for (i = 0; i < nm_i->max_nid; i++)
		if(delfs_test_bit(i, nm_i->nid_bitmap) == 0)
			break;

	ASSERT(i < nm_i->max_nid);
	delfs_set_bit(i, nm_i->nid_bitmap);
	*nid = i;
}

void delfs_release_nid(struct delfs_sb_info *sbi, nid_t nid)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);

	ASSERT(nid < nm_i->max_nid);
	ASSERT(delfs_test_bit(nid, nm_i->nid_bitmap));

	delfs_clear_bit(nid, nm_i->nid_bitmap);
}
...
int delfs_rebuild_qf_inode(struct delfs_sb_info *sbi, int qtype)
{
	...
	ret = write_inode(raw_node, blkaddr, delfs_io_type_to_rw_hint(CURSEG_HOT_NODE));
	if (ret < 0) {
	...
	delfs_clear_bit(ino, F2FS_FSCK(sbi)->nat_area_bitmap);
	delfs_set_bit(ino, NM_I(sbi)->nid_bitmap);
...
{
	__le32 *addr_array;
	struct delfs_node *node_blk = dn->node_blk;
 ...
 */
block_t new_node_block(struct delfs_sb_info *sbi,
				struct dnode_of_data *dn, unsigned int ofs)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_node *delfs_inode;
	...
	delfs_inode = dn->inode_blk;

	node_blk = calloc(F2FS_BLKSIZE, 1);
	ASSERT(node_blk);

	F2FS_NODE_FOOTER(node_blk)->nid = cpu_to_le32(dn->nid);
	F2FS_NODE_FOOTER(node_blk)->ino = F2FS_NODE_FOOTER(delfs_inode)->ino;
	F2FS_NODE_FOOTER(node_blk)->flag = cpu_to_le32(ofs << OFFSET_BIT_SHIFT);
	F2FS_NODE_FOOTER(node_blk)->cp_ver = ckpt->checkpoint_ver;
	set_cold_node(node_blk, S_ISDIR(le16_to_cpu(delfs_inode->i.i_mode)));

	type = CURSEG_COLD_NODE;
	if (IS_DNODE(node_blk)) {
		if (S_ISDIR(le16_to_cpu(delfs_inode->i.i_mode)))
	...
	/* update nat info */
	update_nat_blkaddr(sbi, le32_to_cpu(F2FS_NODE_FOOTER(delfs_inode)->ino),
						dn->nid, blkaddr);
 ...
 */
static int get_node_path(struct delfs_node *node, long block,
...
int get_dnode_of_data(struct delfs_sb_info *sbi, struct dnode_of_data *dn,
						pgoff_t index, int mode)
{
	int offset[4];
	unsigned int noffset[4];
	struct delfs_node *parent = NULL;
	nid_t nids[4];
	block_t nblk[4];
	struct node_info ni;
	int level, i;
	...
	for (i = 1; i <= level; i++) {
		if (!nids[i] && mode == ALLOC_NODE) {
			...
			/* Function new_node_blk get a new delfs_node blk and update*/
			/* We should make sure that dn->node_blk == NULL*/
			nblk[i] = new_node_block(sbi, dn, noffset[i]);
			if (!nblk[i]) {
				delfs_release_nid(sbi, nids[i]);
				c.alloc_failed = 1;
				return -EINVAL;
			}
