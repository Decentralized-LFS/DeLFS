...
int reserve_new_block(struct delfs_sb_info *sbi, block_t *to,
			struct delfs_summary *sum, int type, bool is_inode)
{
	...
	se->valid_blocks++;
	delfs_set_bit(offset, (char *)se->cur_valid_map);
	...
	if (c.func == FSCK) {
		delfs_set_main_bitmap(sbi, blkaddr, type);
		delfs_set_sit_bitmap(sbi, blkaddr);
	}
...
int new_data_block(struct delfs_sb_info *sbi, void *block,
...
u64 delfs_quota_size(struct quota_file *qf)
{
	struct node_info ni;
	struct delfs_node *inode;
	u64 filesize;

	inode = (struct delfs_node *) calloc(F2FS_BLKSIZE, 1);
	ASSERT(inode);
...
u64 delfs_read(struct delfs_sb_info *sbi, nid_t ino, u8 *buffer,
					u64 count, pgoff_t offset)
{
	...
	ASSERT(blk_buffer);
	inode = (struct delfs_node*)(blk_buffer + F2FS_BLKSIZE);
 ...
 *     u64 delfs_write();
 *     u64 delfs_write_compress_data();
 *     u64 delfs_write_addrtag();
 */
static u64 delfs_write_ex(struct delfs_sb_info *sbi, nid_t ino, u8 *buffer,
		u64 count, pgoff_t offset, enum wr_addr_type addr_type)
{
	...
	/*
	 * Enforce calling from delfs_write(), delfs_write_compress_data(),
	 * and delfs_write_addrtag().   Beside, check if is properly called.
	 */
...
u64 delfs_write(struct delfs_sb_info *sbi, nid_t ino, u8 *buffer,
					u64 count, pgoff_t offset)
{
	return delfs_write_ex(sbi, ino, buffer, count, offset, WR_NORMAL);
}

u64 delfs_write_compress_data(struct delfs_sb_info *sbi, nid_t ino, u8 *buffer,
					u64 count, pgoff_t offset)
{
	return delfs_write_ex(sbi, ino, buffer, count, offset, WR_COMPRESS_DATA);
}

u64 delfs_write_addrtag(struct delfs_sb_info *sbi, nid_t ino, pgoff_t offset,
		unsigned int addrtag)
{
	ASSERT(addrtag == COMPRESS_ADDR || addrtag == NEW_ADDR
			|| addrtag == NULL_ADDR);
	return delfs_write_ex(sbi, ino, NULL, F2FS_BLKSIZE, offset, addrtag);
}

/* This function updates only inode->i.i_size */
void delfs_filesize_update(struct delfs_sb_info *sbi, nid_t ino, u64 filesize)
{
	struct node_info ni;
	struct delfs_node *inode;
...
u64 delfs_fix_mutable(struct delfs_sb_info *sbi, nid_t ino, pgoff_t offset,
		unsigned int compressed)
{
	...
	for (i = 0; i < compressed - 1; i++) {
		wlen = delfs_write_addrtag(sbi, ino,
...
static inline void update_extent_info(struct delfs_node *inode,
...
static void update_largest_extent(struct delfs_sb_info *sbi, nid_t ino)
{
	struct dnode_of_data dn;
	struct node_info ni;
	struct delfs_node *inode;
	...
	end_blk  = delfs_max_file_offset(&inode->i) >> F2FS_BLKSIZE_BITS;
...
int delfs_build_file(struct delfs_sb_info *sbi, struct dentry *de)
{
	int fd, n = -1;
	pgoff_t off = 0;
	...
	if (de->size <= DEF_MAX_INLINE_DATA) {
		...
		while (!eof && (n = bulkread(fd, rbuf, c.compress.cc.rlen,
			...
			if (ret || n < c.compress.cc.rlen ||
				...
				wlen = delfs_write(sbi, de->ino, rbuf, n, off);
				ASSERT((int)wlen == n);
			} else {
				wlen = delfs_write_addrtag(sbi, de->ino, off,
						WR_COMPRESS_ADDR);
				ASSERT(!wlen);
				wlen = delfs_write_compress_data(sbi, de->ino,
				...
				cur_cblk = (c.compress.cc.rlen - csize) /
								F2FS_BLKSIZE;
				cblocks += cur_cblk;
				wlen = delfs_fix_mutable(sbi, de->ino,
		...
		while ((n = read(fd, buffer, F2FS_BLKSIZE)) > 0) {
			delfs_write(sbi, de->ino, buffer, n, off);
			off += n;
		}
	}
		...
		struct delfs_node *node_blk)
{
	...
	se->dirty = 1;
	delfs_clear_main_bitmap(sbi, old_blkaddr);
	delfs_clear_sit_bitmap(sbi, old_blkaddr);
	...
	ret = dev_write_block(buf, new_blkaddr, delfs_io_type_to_rw_hint(type));
	ASSERT(ret >= 0);
	...
	*blkaddr = new_blkaddr;
	...
	if (need_fsync_data_record(sbi))
		delfs_set_bit(offset, (char *)se->ckpt_valid_map);
	se->dirty = 1;
	delfs_set_main_bitmap(sbi, new_blkaddr, type);
	delfs_set_sit_bitmap(sbi, new_blkaddr);
