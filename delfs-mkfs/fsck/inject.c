...
static void print_raw_nat_entry_info(struct delfs_nat_entry *ne)
{
	if (!c.dbg_lv)
		return;
...
static void print_raw_sit_entry_info(struct delfs_sit_entry *se)
{
	int i;
...
static void print_raw_sum_entry_info(struct delfs_summary *sum)
{
	if (!c.dbg_lv)
		return;
...
static void print_raw_dentry_info(struct delfs_dir_entry *dentry)
{
	if (!c.dbg_lv)
		return;
...
{
	MSG(0, "\nUsage: inject.delfs [options] device\n");
	MSG(0, "[options]:\n");
...
{
	MSG(0, "inject.delfs --sb <0|1|2> --mb <name> [--idx <index>] --val/str <value/string>\n");
	MSG(0, "[sb]:\n");
...
{
	MSG(0, "inject.delfs --cp <0|1|2> --mb <name> [--idx <index>] --val <value> inject checkpoint\n");
	MSG(0, "[cp]:\n");
...
{
	MSG(0, "inject.delfs --nat <0|1|2> --mb <name> --nid <nid> --val <value> inject nat entry\n");
	MSG(0, "[nat]:\n");
...
{
	MSG(0, "inject.delfs --sit <0|1|2> --mb <name> --blk <blk> [--idx <index>] --val <value> inject sit entry\n");
	MSG(0, "[sit]:\n");
...
{
	MSG(0, "inject.delfs --ssa --mb <name> --blk <blk> [--idx <index>] --val <value> inject summary entry\n");
	MSG(0, "[mb]:\n");
...
{
	MSG(0, "inject.delfs --node --mb <name> --nid <nid> [--idx <index>] --val <value> inject node\n");
	MSG(0, "[mb]:\n");
...
{
	MSG(0, "inject.delfs --dent --mb <name> --nid <nid> [--idx <index>] --val <value> inject dentry\n");
	MSG(0, "[mb]:\n");
...
static int inject_sb(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	struct delfs_super_block *sb;
	char *buf;
	int ret;
	...
	sb = (struct delfs_super_block *)(buf + F2FS_SUPER_OFFSET);
...
static int inject_cp(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	struct delfs_checkpoint *cp, *cur_cp = F2FS_CKPT(sbi);
	char *buf = NULL;
	int ret = 0;
	...
	if (opt->cp != sbi->cur_cp) {
		...
		cp = (struct delfs_checkpoint *)buf;
		sbi->ckpt = cp;
		sbi->cur_cp = opt->cp;
	} else {
		cp = cur_cp;
	...
	if (!strcmp(opt->mb, "checkpoint_ver")) {
		...
		MSG(0, "Info: inject cur_node_segno[%d] of cp %d: 0x%x -> 0x%x\n",
		    opt->idx, opt->cp, get_cp(cur_nodes_segno[0][opt->idx]),
		    (u32)opt->val);
		set_cp(cur_nodes_segno[0][opt->idx], (u32)opt->val);
		...
		MSG(0, "Info: inject cur_node_blkoff[%d] of cp %d: 0x%x -> 0x%x\n",
		    opt->idx, opt->cp, get_cp(cur_nodes_blkoff[0][opt->idx]),
		    (u16)opt->val);
		set_cp(cur_nodes_blkoff[0][opt->idx], (u16)opt->val);
		...
		MSG(0, "Info: inject cur_data_segno[%d] of cp %d: 0x%x -> 0x%x\n",
		    opt->idx, opt->cp, get_cp(cur_datas_segno[0][opt->idx]),
		    (u32)opt->val);
		set_cp(cur_datas_segno[0][opt->idx], (u32)opt->val);
		...
		MSG(0, "Info: inject cur_data_blkoff[%d] of cp %d: 0x%x -> 0x%x\n",
		    opt->idx, opt->cp, get_cp(cur_datas_blkoff[0][opt->idx]),
		    (u16)opt->val);
		set_cp(cur_datas_blkoff[0][opt->idx], (u16)opt->val);
	} else {
		ERR_MSG("unknown or unsupported member \"%s\"\n", opt->mb);
		ret = -EINVAL;
		goto out;
...
static int inject_nat(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	struct delfs_nm_info *nm_i = NM_I(sbi);
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_nat_block *nat_blk;
	...
	is_set = delfs_test_bit(opt->nid, nm_i->nat_bitmap);
	if (opt->nat == 0) {
		opt->nat = is_set ? 2 : 1;
	} else {
		if (opt->nat == 1)
			delfs_clear_bit(opt->nid, nm_i->nat_bitmap);
		else
			delfs_set_bit(opt->nid, nm_i->nat_bitmap);
	}
...
static int inject_sit(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	...
	if (!delfs_is_valid_blkaddr(sbi, opt->blk, DATA_GENERIC)) {
	...
	is_set = delfs_test_bit(segno, sit_i->sit_bitmap);
	if (opt->sit == 0) {
		opt->sit = is_set ? 2 : 1;
	} else {
		if (opt->sit == 1)
			delfs_clear_bit(segno, sit_i->sit_bitmap);
		else
			delfs_set_bit(segno, sit_i->sit_bitmap);
	}
...
static int inject_ssa(struct delfs_sb_info *sbi, struct inject_option *opt)
{
...
static int inject_inode(struct delfs_sb_info *sbi, struct delfs_node *node,
...
static int inject_index_node(struct delfs_sb_info *sbi, struct delfs_node *node,
			     struct inject_option *opt)
{
...
static int inject_node(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	struct delfs_super_block *sb = sbi->raw_super;
	struct node_info ni;
...
static int find_dir_entry(struct delfs_dentry_ptr *d, nid_t ino)
{
	struct delfs_dir_entry *de;
	int slot = 0;
...
static int inject_dentry(struct delfs_sb_info *sbi, struct inject_option *opt)
{
	struct node_info ni;
	struct delfs_node *node_blk = NULL;
	struct delfs_inode *inode;
	struct delfs_dentry_ptr d;
	void *inline_dentry;
	struct delfs_dentry_block *dent_blk = NULL;
	block_t addr = 0;
	void *buf = NULL;
	struct delfs_dir_entry *dent = NULL;
	...
	if (inode->i_inline & F2FS_INLINE_DENTRY) {
		...
		for (i = 0; i < end; i++) {
			...
			if (addr == NULL_ADDR || addr == NEW_ADDR)
				continue;
			if (!delfs_is_valid_blkaddr(sbi, addr, DATA_GENERIC)) {
...
int do_inject(struct delfs_sb_info *sbi)
{
