					...
					void *buf, unsigned int size)
{
	...
	written = delfs_write(qf->sbi, qf->ino, buf, size, offset);
...
static unsigned int quota_read_nomount(struct quota_file *qf, long offset,
		void *buf, unsigned int size)
{
	return delfs_read(qf->sbi, qf->ino, buf, size, offset);
}
...
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	delfs_ino_t qf_inum = sb->qf_ino[qtype];
	errcode_t err = 0;
		...
		int update_filesize)
{
	...
	if (h->qh_ops->end_io && h->qh_ops->end_io(h) < 0)
		return EIO;
	if (update_filesize) {
		delfs_filesize_update(sbi, h->qh_qf.ino, h->qh_qf.filesize);
	}
