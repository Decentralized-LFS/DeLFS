...
void *read_all_xattrs(struct delfs_sb_info *sbi, struct delfs_node *inode,
			bool sanity_check)
{
	struct delfs_xattr_header *header;
	void *txattr_addr;
...
static struct delfs_xattr_entry *__find_xattr(void *base_addr,
				void *last_base_addr, int index,
				size_t len, const char *name)
{
	struct delfs_xattr_entry *entry;
...
int delfs_setxattr(struct delfs_sb_info *sbi, nid_t ino, int index, const char *name,
		const void *value, size_t size, int flags)
{
	struct delfs_node *inode;
	void *base_addr;
	void *last_base_addr;
	struct delfs_xattr_entry *here, *last;
	struct node_info ni;
	int error = 0;
	int len;
	int found, newsize;
	...
	newsize = XATTR_ALIGN(sizeof(struct delfs_xattr_entry) + len + size);
	...
	if (found) {
		 ...
		 */
		struct delfs_xattr_entry *next = XATTR_NEXT_ENTRY(here);
		int oldsize = ENTRY_SIZE(here);

		memmove(here, next, (char *)last - (char *)next);
		last = (struct delfs_xattr_entry *)((char *)last - oldsize);
...
int inode_set_selinux(struct delfs_sb_info *sbi, u32 ino, const char *secon)
{
	if (!secon)
		return 0;

	return delfs_setxattr(sbi, ino, F2FS_XATTR_INDEX_SECURITY,
