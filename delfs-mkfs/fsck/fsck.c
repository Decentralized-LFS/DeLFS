...
int delfs_set_main_bitmap(struct delfs_sb_info *sbi, u32 blk, int type)
{
	...
	return delfs_set_bit(BLKOFF_FROM_MAIN(sbi, blk), fsck->main_area_bitmap);
}

static inline int delfs_test_main_bitmap(struct delfs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return delfs_test_bit(BLKOFF_FROM_MAIN(sbi, blk),
						fsck->main_area_bitmap);
}

int delfs_clear_main_bitmap(struct delfs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return delfs_clear_bit(BLKOFF_FROM_MAIN(sbi, blk),
						fsck->main_area_bitmap);
}

static inline int delfs_test_sit_bitmap(struct delfs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return delfs_test_bit(BLKOFF_FROM_MAIN(sbi, blk), fsck->sit_area_bitmap);
}

int delfs_set_sit_bitmap(struct delfs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return delfs_set_bit(BLKOFF_FROM_MAIN(sbi, blk), fsck->sit_area_bitmap);
}

int delfs_clear_sit_bitmap(struct delfs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
...
static int add_into_hard_link_list(struct delfs_sb_info *sbi,
...
static int find_and_dec_hard_link_list(struct delfs_sb_info *sbi, u32 nid)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
...
static int is_valid_ssa_node_blk(struct delfs_sb_info *sbi, u32 nid,
							u32 blk_addr)
{
	...
	if (need_fix && delfs_dev_is_writable()) {
		u64 ssa_blk;
		int ret2;
...
static int is_valid_summary(struct delfs_sb_info *sbi, struct delfs_summary *sum,
							u32 blk_addr)
{
	u16 ofs_in_node = le16_to_cpu(sum->ofs_in_node);
	u32 nid = le32_to_cpu(sum->nid);
	struct delfs_node *node_blk = NULL;
	...
	if (!delfs_is_valid_blkaddr(sbi, ni.blk_addr, DATA_GENERIC))
		goto out;
...
static int is_valid_ssa_data_blk(struct delfs_sb_info *sbi, u32 blk_addr,
...
static int __check_inode_mode(u32 nid, enum FILE_TYPE ftype, u16 mode)
{
	if (ftype >= F2FS_FT_MAX)
		return 0;
	/* delfs_iget will return -EIO if mode is not valid file type */
...
static int sanity_check_nat(struct delfs_sb_info *sbi, u32 nid,
						struct node_info *ni)
{
	if (!IS_VALID_NID(sbi, nid)) {
...
int fsck_sanity_check_nat(struct delfs_sb_info *sbi, u32 nid)
{
	struct node_info ni;
...
static int sanity_check_nid(struct delfs_sb_info *sbi, u32 nid,
			...
			struct node_info *ni)
{
	...
	if ((ntype == TYPE_INODE && ftype == F2FS_FT_DIR) ||
		...
		/* not included '.' & '..' */
		if (delfs_test_main_bitmap(sbi, ni->blk_addr) != 0) {
	...
	if (delfs_test_sit_bitmap(sbi, ni->blk_addr) == 0)
...
int fsck_sanity_check_nid(struct delfs_sb_info *sbi, u32 nid,
			enum FILE_TYPE ftype, enum NODE_TYPE ntype)
{
	struct delfs_node *node_blk = NULL;
	struct node_info ni;
...
static int fsck_chk_xattr_blk(struct delfs_sb_info *sbi, u32 ino,
					u32 x_nid, u32 *blk_cnt)
{
	...
	*blk_cnt = *blk_cnt + 1;
	delfs_set_main_bitmap(sbi, ni.blk_addr, CURSEG_COLD_NODE);
...
int fsck_chk_node_blk(struct delfs_sb_info *sbi, struct delfs_inode *inode,
		u32 nid, enum FILE_TYPE ftype, enum NODE_TYPE ntype,
		u32 *blk_cnt, struct delfs_compr_blk_cnt *cbc,
		struct child_info *child)
{
	...
	if (ntype == TYPE_INODE) {
		...
		case TYPE_DIRECT_NODE:
			delfs_set_main_bitmap(sbi, ni.blk_addr,
							CURSEG_WARM_NODE);
...
int fsck_chk_root_inode(struct delfs_sb_info *sbi)
{
	struct delfs_node *node_blk;
...
fix:
	if (!last_blkaddr) {
		...
		if (delfs_test_bit(root_ino, fsck->nat_area_bitmap))
			delfs_clear_bit(root_ino, fsck->nat_area_bitmap);
		fsck->chk.valid_nat_entry_cnt++;

		if (!delfs_test_sit_bitmap(sbi, last_blkaddr))
			delfs_set_sit_bitmap(sbi, last_blkaddr);
		ret = 0;
	}
	free(node_blk);
	return ret;
...
void fsck_reada_node_block(struct delfs_sb_info *sbi, u32 nid)
{
	struct node_info ni;
...
void fsck_reada_all_direct_node_blocks(struct delfs_sb_info *sbi,
						struct delfs_node *node_blk)
{
	int i;
...
int chk_extended_attributes(struct delfs_sb_info *sbi, u32 nid,
		struct delfs_node *inode)
{
	void *xattr;
	void *last_base_addr;
...
void fsck_chk_inode_blk(struct delfs_sb_info *sbi, u32 nid,
		enum FILE_TYPE ftype, struct delfs_node *node_blk,
		u32 *blk_cnt, struct delfs_compr_blk_cnt *cbc,
...
check_next:
	...
	if (ftype == F2FS_FT_DIR) {
		delfs_set_main_bitmap(sbi, ni->blk_addr, CURSEG_HOT_NODE);
	...
	if (delfs_has_extra_isize(&node_blk->i)) {
		...
		if (child.dot == 0 || child.dotdot == 0) {
			...
			if (c.fix_on) {
				...
				if (child.dot == 0) {
					...
					ret = delfs_add_link(sbi, node_blk,
...
skip_dot_fix:
	...
	if (need_fix && delfs_dev_is_writable() && !is_aliasing)
		node_blk->i.i_ext.len = 0;

	if ((c.feature & F2FS_FEATURE_INODE_CHKSUM) &&
				delfs_has_extra_isize(&node_blk->i)) {
		__u32 provided, calculated;

		provided = le32_to_cpu(node_blk->i.i_inode_checksum);
		calculated = delfs_inode_chksum(node_blk);
...
int fsck_chk_dnode_blk(struct delfs_sb_info *sbi, struct delfs_inode *inode,
		u32 nid, enum FILE_TYPE ftype, struct delfs_node *node_blk,
		u32 *blk_cnt, struct delfs_compr_blk_cnt *cbc,
...
int fsck_chk_idnode_blk(struct delfs_sb_info *sbi, struct delfs_inode *inode,
		enum FILE_TYPE ftype, struct delfs_node *node_blk, u32 *blk_cnt,
		struct delfs_compr_blk_cnt *cbc, struct child_info *child)
{
	int need_fix = 0, ret;
	int i = 0;
...
int fsck_chk_didnode_blk(struct delfs_sb_info *sbi, struct delfs_inode *inode,
		enum FILE_TYPE ftype, struct delfs_node *node_blk, u32 *blk_cnt,
		struct delfs_compr_blk_cnt *cbc, struct child_info *child)
{
	int i = 0;
...
static void print_dentry(struct delfs_sb_info *sbi, __u8 *name,
		u8 *bitmap, struct delfs_dir_entry *dentry,
		int max, int idx, int last_blk, int enc_name)
{
	...
	if (c.show_file_map) {
		struct delfs_dentry *d = fsck->dentry;
...
static int delfs_check_hash_code(int encoding, int casefolded,
			struct delfs_dir_entry *dentry,
			const unsigned char *name, u32 len, int enc_name)
{
	...
	delfs_hash_t hash_code = delfs_dentry_hash(encoding, casefolded, name, len);
...
static int delfs_check_dirent_position(const struct delfs_dir_entry *dentry,
...
static int __chk_dots_dentries(struct delfs_sb_info *sbi,
			       int casefolded,
			       struct delfs_dir_entry *dentry,
			       struct child_info *child,
			       u8 *name, int len,
			       ...
			       int enc_name)
{
	...
	if (delfs_check_hash_code(get_encoding(sbi), casefolded, dentry, name, len, enc_name))
		fixed = 1;
...
static void nullify_dentry(struct delfs_dir_entry *dentry, int offs,
			   __u8 (*filename)[F2FS_SLOT_LEN], u8 **bitmap)
{
	memset(dentry, 0, sizeof(struct delfs_dir_entry));
	test_and_clear_bit_le(offs, *bitmap);
	memset(*filename, 0, F2FS_SLOT_LEN);
}

static int __chk_dentries(struct delfs_sb_info *sbi, int casefolded,
			struct child_info *child,
			u8 *bitmap, struct delfs_dir_entry *dentry,
			...
			int max, int last_blk, int enc_name)
{
	...
	int dentries = 0;
	u32 blk_cnt;
	struct delfs_compr_blk_cnt cbc;
	u8 *name;
	...
	for (i = 0; i < max; i++) {
		...
		if (IS_VALID_NID(sbi, ino)) {
			...
			if (delfs_is_valid_blkaddr(sbi, ni.blk_addr,
							DATA_GENERIC)) {
				dev_reada_block(ni.blk_addr);
	...
	for (i = 0; i < max;) {
		...
		if (delfs_check_hash_code(get_encoding(sbi), casefolded, dentry + i, name, name_len, enc_name))
			fixed = 1;

		pretty_print_filename(name, name_len, en, enc_name);

		if (max == NR_DENTRY_IN_BLOCK) {
			ret = delfs_check_dirent_position(dentry + i, en,
...
int fsck_chk_inline_dentries(struct delfs_sb_info *sbi,
		struct delfs_node *node_blk, struct child_info *child)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_dentry *cur_dentry = fsck->dentry_end;
	struct delfs_dentry *new_dentry;
	struct delfs_dentry_ptr d;
	void *inline_dentry;
	int dentries;
	...
	fsck->dentry_depth++;
	new_dentry = calloc(sizeof(struct delfs_dentry), 1);
...
int fsck_chk_dentry_blk(struct delfs_sb_info *sbi, int casefolded, u32 blk_addr,
		struct child_info *child, int last_blk, int enc_name,
		struct delfs_node *node_blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_dentry_block *de_blk;
	struct delfs_dentry *cur_dentry = fsck->dentry_end;
	struct delfs_dentry *new_dentry;
	int dentries, ret;

	de_blk = (struct delfs_dentry_block *)calloc(F2FS_BLKSIZE, 1);
	ASSERT(de_blk != NULL);
	...
	if (dentries < 0 && delfs_dev_is_writable()) {
...
int fsck_chk_data_blk(struct delfs_sb_info *sbi, struct delfs_inode *inode,
		...
		struct delfs_node *node_blk)
{
	...
	if (!delfs_is_valid_blkaddr(sbi, blk_addr, DATA_GENERIC)) {
	...
	if (delfs_test_sit_bitmap(sbi, blk_addr) == 0)
		ASSERT_MSG("SIT bitmap is 0x0. blk_addr[0x%x]", blk_addr);

	if (delfs_test_main_bitmap(sbi, blk_addr) != 0)
	...
	if (ftype == F2FS_FT_DIR) {
		delfs_set_main_bitmap(sbi, blk_addr, CURSEG_HOT_DATA);
		return fsck_chk_dentry_blk(sbi, casefolded, blk_addr, child,
				last_blk, enc_name, node_blk);
	} else {
		delfs_set_main_bitmap(sbi, blk_addr, CURSEG_WARM_DATA);
	}
	return 0;
}

int fsck_chk_orphan_node(struct delfs_sb_info *sbi)
{
	u32 blk_cnt = 0;
	struct delfs_compr_blk_cnt cbc = {0, CHEADER_PGOFS_NONE};
	block_t start_blk, orphan_blkaddr, i, j;
	struct delfs_orphan_block *orphan_blk, *new_blk;
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	u32 entry_count;
	...
	delfs_ra_meta_pages(sbi, start_blk, orphan_blkaddr, META_CP);
	...
	for (i = 0; i < orphan_blkaddr; i++) {
		...
		for (j = 0; j < entry_count; j++) {
			...
			if (c.preen_mode == PREEN_MODE_1 && !c.fix_on) {
				get_node_info(sbi, ino, &ni);
				if (!IS_VALID_NID(sbi, ino) ||
					!delfs_is_valid_blkaddr(sbi, ni.blk_addr,
								DATA_GENERIC)) {
					free(orphan_blk);
					free(new_blk);
					return -EINVAL;
		...
		if (delfs_dev_is_writable() && c.fix_on &&
...
int fsck_chk_quota_node(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		if (ret) {
			...
			if (c.fix_on)
				delfs_rebuild_qf_inode(sbi, qtype);
		}
	}
	cur_qtype = -1;
	return ret;
...
static void fsck_disconnect_file(struct delfs_sb_info *sbi, nid_t ino,
			bool dealloc);

int fsck_chk_quota_files(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	enum quota_type qtype;
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		if (c.fix_on) {
			...
			delfs_rebuild_qf_inode(sbi, qtype);
			delfs_filesize_update(sbi, ino, 0);
...
int fsck_chk_meta(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	for (i = 0; i < fsck->nr_nat_entries; i++) {
		...
		if (!delfs_is_valid_blkaddr(sbi, blk, DATA_GENERIC)) {
		...
		if (!delfs_test_sit_bitmap(sbi, blk)) {
			MSG(0, "\tError: nat entry[ino %u block_addr 0x%x]"
		...
		if (!delfs_test_bit(ino, fsck->nat_area_bitmap)) {
...
void fsck_chk_checkpoint(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
void fsck_init(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	ASSERT(tree_mark != NULL);
	fsck->dentry = calloc(sizeof(struct delfs_dentry), 1);
...
static void fix_hard_links(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	while (node) {
		...
		if (sanity_check_nid(sbi, node->nid, node_blk,
			...
			FIX_MSG("Failed to fix, rerun fsck.delfs");
...
static void fix_nat_entries(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	u32 i;

	for (i = 0; i < fsck->nr_nat_entries; i++)
		if (delfs_test_bit(i, fsck->nat_area_bitmap) != 0)
			nullify_nat_entry(sbi, i);
}

static void flush_curseg_sit_entries(struct delfs_sb_info *sbi)
{
...
static void fix_checksum(struct delfs_sb_info *sbi)
{
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
static void fix_checkpoint(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
...
static void fix_checkpoints(struct delfs_sb_info *sbi)
{
 ...
 */
static int last_vblk_off_in_zone(struct delfs_sb_info *sbi,
				 unsigned int zone_segno)
{
	...
	for (s = segs_per_zone - 1; s >= 0; s--) {
		...
		for (b = sbi->blocks_per_seg - 1; b >= 0; b--)
			if (delfs_test_bit(b, (const char *)se->cur_valid_map))
...
static int check_curseg_write_pointer(struct delfs_sb_info *sbi, int type)
{
	...
	ret = delfs_report_zone(i, cs_sector, &blkz);
	if (ret)
		return ret;
...
static int check_curseg_write_pointer(struct delfs_sb_info *UNUSED(sbi),
					int UNUSED(type))
{
	return 0;
}
...
int check_curseg_offset(struct delfs_sb_info *sbi, int type, bool check_wp)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct curseg_info *curseg = CURSEG_I(sbi, type);
	struct seg_entry *se;
	...
	se = get_seg_entry(sbi, curseg->segno);
	if (delfs_test_bit(curseg->next_blkoff,
	...
	for (j = curseg->next_blkoff + 1; j < nblocks; j++) {
		if (delfs_test_bit(j, (const char *)se->cur_valid_map)) {
...
int check_curseg_offsets(struct delfs_sb_info *sbi, bool check_wp)
{
	int i, ret;
...
static void fix_curseg_info(struct delfs_sb_info *sbi, bool check_wp)
{
	int i, need_update = 0;
...
int check_sit_types(struct delfs_sb_info *sbi)
{
	unsigned int i;
	int err = 0;
...
static struct delfs_node *fsck_get_lpf(struct delfs_sb_info *sbi)
{
	struct delfs_node *node;
	struct node_info ni;
	nid_t lpf_ino;
	int err;
	...
	lpf_ino = delfs_lookup(sbi, node, (u8 *)LPF, strlen(LPF));
	if (lpf_ino) { /* found */
...
static int fsck_do_reconnect_file(struct delfs_sb_info *sbi,
				  struct delfs_node *lpf,
				  struct delfs_node *fnode)
{
	char name[80];
	size_t namelen;
	...
	if (delfs_lookup(sbi, lpf, (u8 *)name, namelen)) {
	...
	ret = delfs_add_link(sbi, lpf, (unsigned char *)name, namelen,
...
static inline void release_inode_cnt(struct delfs_sb_info *sbi, bool dealloc)
{
...
static inline void release_node_cnt(struct delfs_sb_info *sbi, bool dealloc)
{
...
static inline void release_block_cnt(struct delfs_sb_info *sbi, bool dealloc)
{
	F2FS_FSCK(sbi)->chk.valid_blk_cnt--;
	if (dealloc)
...
static inline void release_block(struct delfs_sb_info *sbi, u64 blkaddr,
			bool dealloc)
{
	delfs_clear_main_bitmap(sbi, blkaddr);
	if (dealloc) {
		struct seg_entry *se;
		u64 offset;
		...
		se->valid_blocks--;
		delfs_clear_bit(offset, (char *)se->cur_valid_map);
		if (need_fsync_data_record(sbi))
			delfs_clear_bit(offset, (char *)se->ckpt_valid_map);
		se->dirty = 1;
		delfs_clear_sit_bitmap(sbi, blkaddr);
	}
}

static inline void release_nat_entry(struct delfs_sb_info *sbi, u32 nid)
{
	nullify_nat_entry(sbi, nid);
	F2FS_FSCK(sbi)->chk.valid_nat_entry_cnt--;
}

static void fsck_disconnect_file_dnode(struct delfs_sb_info *sbi,
			struct delfs_inode *inode, nid_t nid, bool dealloc)
{
	struct delfs_node *node;
	struct node_info ni;
	u32 addr;
...
static void fsck_disconnect_file_idnode(struct delfs_sb_info *sbi,
			struct delfs_inode *inode, nid_t nid, bool dealloc)
{
	struct delfs_node *node;
	struct node_info ni;
...
static void fsck_disconnect_file_didnode(struct delfs_sb_info *sbi,
			struct delfs_inode *inode, nid_t nid, bool dealloc)
{
	struct delfs_node *node;
	struct node_info ni;
 ...
 */
static int fsck_reconnect_file(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct delfs_node *lpf_node, *node;
	struct node_info ni;
	...
	for (nid = 0; nid < fsck->nr_nat_entries; nid++) {
		if (delfs_test_bit(nid, fsck->nat_area_bitmap)) {
			if (is_qf_ino(F2FS_RAW_SUPER(sbi), nid)) {
			...
			delfs_set_bit(nid, reconnect_bitmap);
		}
	}
	...
	for (nid = 0; nid < fsck->nr_nat_entries; nid++) {
		if (delfs_test_bit(nid, reconnect_bitmap)) {
			get_node_info(sbi, nid, &ni);
			err = dev_read_block(node, ni.blk_addr);
			ASSERT(err >= 0);
...
static int chk_and_fix_wp_with_sit(int UNUSED(i), void *blkzone, void *opaque)
{
	...
	struct delfs_sb_info *sbi = wpd->sbi;
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		if (zone_segno <= cs->segno &&
			...
			if (last_valid_blkoff < 0 &&
				...
				ret = delfs_reset_zone(wpd->dev_index, blkz);
				if (ret) {
	...
	ret = delfs_finish_zone(wpd->dev_index, blkz);
	if (ret) {
		...
		ret = dev_fill(NULL, wp_block * F2FS_BLKSIZE,
			...
			delfs_io_type_to_rw_hint(se->type));
		if (ret)
...
static void fix_wp_sit_alignment(struct delfs_sb_info *sbi)
{
	unsigned int i;
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		...
		wpd.dev_index = i;
		if (delfs_report_zones(i, chk_and_fix_wp_with_sit, &wpd)) {
...
static void fix_wp_sit_alignment(struct delfs_sb_info *UNUSED(sbi))
{
	return;
}
 ...
 */
void fsck_chk_and_fix_write_pointers(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
...
int fsck_chk_curseg_info(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
int fsck_verify(struct delfs_sb_info *sbi)
{
	unsigned int i = 0;
	int ret = 0;
	int force = 0;
	...
	if (nr_unref_nid && !c.ro) {
		...
		if (!strcasecmp(ans, "y")) {
			for (i = 0; i < fsck->nr_nat_entries; i++) {
				if (delfs_test_bit(i, fsck->nat_area_bitmap))
	...
	/* fix global metadata */
	if (force || (c.fix_on && delfs_dev_is_writable())) {
		struct delfs_checkpoint *cp = F2FS_CKPT(sbi);
		struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
void fsck_free(struct delfs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	while (fsck->dentry) {
		struct delfs_dentry *dentry = fsck->dentry;
