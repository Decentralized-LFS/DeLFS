...
void fsck_usage()
{
	MSG(0, "\nUsage: fsck.delfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -a check/fix potential corruption, reported by delfs\n");
...
void dump_usage()
{
	MSG(0, "\nUsage: dump.delfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -d debug level [default:0]\n");
...
void defrag_usage()
{
	MSG(0, "\nUsage: defrag.delfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -d debug level [default:0]\n");
	MSG(0, "  -H support write hint\n");
...
void resize_usage()
{
	MSG(0, "\nUsage: resize.delfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -d debug level [default:0]\n");
	MSG(0, "  -H support write hint\n");
...
void sload_usage()
{
	MSG(0, "\nUsage: sload.delfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -C fs_config\n");
...
void label_usage()
{
	MSG(0, "\nUsage: delfslabel [options] device [volume-label]\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -V print the version number and exit\n");
	exit(1);
}
...
{
	if (!strcmp("fsck.delfs", prog))
		fsck_usage();
	else if (!strcmp("dump.delfs", prog))
		dump_usage();
	else if (!strcmp("defrag.delfs", prog))
		defrag_usage();
	else if (!strcmp("resize.delfs", prog))
		resize_usage();
	else if (!strcmp("sload.delfs", prog))
		sload_usage();
	else if (!strcmp("delfslabel", prog))
		label_usage();
	else if (!strcmp("inject.delfs", prog))
		inject_usage();
	else
		MSG(0, "\nWrong program.\n");
}
...
void delfs_parse_options(int argc, char *argv[])
{
	int option = 0;
	...
	/* Allow prog names (e.g, sload_delfs, fsck_delfs, etc) */
	...
	if (!strcmp("fsck.delfs", prog)) {
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case 'C':
				...
				c.s_encoding = val;
				token = strtok(NULL, "");
				val = delfs_str2encoding_flags(&token, &c.s_encoding_flags);
				if (val) {
...
static int do_fsck(struct delfs_sb_info *sbi)
{
...
#ifdef WITH_DUMP
static void do_dump(struct delfs_sb_info *sbi)
{
...
#ifdef WITH_DEFRAG
static int do_defrag(struct delfs_sb_info *sbi)
{
	...
	return delfs_defragment(sbi, c.defrag_start, c.defrag_len,
...
#ifdef WITH_RESIZE
static int do_resize(struct delfs_sb_info *sbi)
{
	if (!c.target_sectors)
	...
	return delfs_resize(sbi);
}
#endif

#ifdef WITH_SLOAD
static int init_compr(struct delfs_sb_info *sbi)
{
...
static int do_sload(struct delfs_sb_info *sbi)
{
	if (!c.from_dir) {
	...
	return delfs_sload(sbi);
}
#endif

#ifdef WITH_LABEL
static int do_label(struct delfs_sb_info *sbi)
{
	struct delfs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
int main(int argc, char **argv)
{
	...
	delfs_init_configuration();

	delfs_parse_options(argc, argv);

	if (c.func != DUMP && (ret = delfs_devs_are_umounted()) < 0) {
		if (ret == -EBUSY) {
			ret = -1;
			if (c.func == FSCK)
	...
	/* Get device */
	if (delfs_get_device_info() < 0 || delfs_get_delfs_info() < 0) {
		ret = -1;
		if (c.func == FSCK)
			ret = FSCK_OPERATIONAL_ERROR;
		goto quick_err;
...
fsck_again:
	...
	case SLOAD:
		...
		ret = delfs_sparse_initialize_meta(sbi);
		if (ret < 0)
			goto out_err;
