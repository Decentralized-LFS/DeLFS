...
static inline bool IS_INODE(struct delfs_node *node)
{
	return F2FS_NODE_FOOTER(node)->nid == F2FS_NODE_FOOTER(node)->ino;
}

static inline unsigned int ADDRS_PER_PAGE(struct delfs_sb_info *sbi,
		struct delfs_node *node_blk, struct delfs_node *inode_blk)
{
...
static inline __le32 *blkaddr_in_inode(struct delfs_node *node)
{
	return node->i.i_addr + get_extra_isize(node);
}

static inline __le32 *blkaddr_in_node(struct delfs_node *node)
{
	return IS_INODE(node) ? blkaddr_in_inode(node) : node->dn.addr;
}

static inline block_t datablock_addr(struct delfs_node *node_page,
					unsigned int offset)
{
	__le32 *addr_array;
...
static inline void set_nid(struct delfs_node * rn, int off, nid_t nid, int i)
{
	if (i)
...
static inline nid_t get_nid(struct delfs_node * rn, int off, int i)
{
	if (i)
...
static inline void set_new_dnode(struct dnode_of_data *dn,
		struct delfs_node *iblk, struct delfs_node *nblk, nid_t nid)
{
...
static inline int IS_DNODE(struct delfs_node *node_page)
{
...
static inline nid_t ino_of_node(struct delfs_node *node_blk)
{
	return le32_to_cpu(F2FS_NODE_FOOTER(node_blk)->ino);
}

static inline __u64 cpver_of_node(struct delfs_node *node_blk)
{
	return le64_to_cpu(F2FS_NODE_FOOTER(node_blk)->cp_ver);
}

static inline bool is_recoverable_dnode(struct delfs_sb_info *sbi,
						struct delfs_node *node_blk)
{
	struct delfs_checkpoint *ckpt = F2FS_CKPT(sbi);
	__u64 cp_ver = cur_cp_version(ckpt);

	/* Don't care crc part, if fsck.delfs sets it. */
...
static inline block_t next_blkaddr_of_node(struct delfs_node *node_blk)
{
	return le32_to_cpu(F2FS_NODE_FOOTER(node_blk)->next_blkaddr);
}

static inline int is_node(struct delfs_node *node_blk, int type)
{
	return le32_to_cpu(F2FS_NODE_FOOTER(node_blk)->flag) & (1 << type);
}

static inline void set_cold_node(struct delfs_node *rn, bool is_dir)
{
