...
/*
 * code borrowed from kernel delfs dirver: f2fs.h, GPL-2.0
...
#define CP_CHKSUM_OFFSET	(((F2FS_BLKSIZE) * (4)) - sizeof(__le32))
...
#define	F2FS_NUMBER_OF_CHECKPOINT_PACK	4
...
/* delfs_configration for compression used for sload.delfs */
typedef struct  {
...
/*
 * Copied from fs/delfs/segment.h
 */
...
/*
 * Copied from include/linux/delfs_sb.h
 */
...
/*
 * On-disk inode flags (delfs_inode::i_flags)
 */
...
static_assert(sizeof(struct delfs_device) == 68, "");
...
struct delfs_super_block {
	...
	struct delfs_device devs[MAX_DEVICES] __attribute__((packed));	/* device list */
...
static_assert(sizeof(struct delfs_super_block) == 3072, "");
...
#define F2FS_CP_PACKS		8	/* # of checkpoint packs */

struct delfs_checkpoint {
	...
	/* taehwan code */
//	__le16 compact_page_count;
//	__le32 tmp_reserve[1500];
	__le32 cur_nodes_segno[MAX_CORE_COUNT][MAX_ACTIVE_NODE_LOGS];
	__le16 cur_nodes_blkoff[MAX_CORE_COUNT][MAX_ACTIVE_NODE_LOGS];

	__le32 cur_datas_segno[MAX_CORE_COUNT][MAX_ACTIVE_NODE_LOGS];
	__le16 cur_datas_blkoff[MAX_CORE_COUNT][MAX_ACTIVE_NODE_LOGS];
	/* taehwan code */
...
//static_assert(sizeof(struct delfs_checkpoint) == 192, "");
//static_assert(sizeof(struct delfs_checkpoint) == 192+6000, "");

#define CP_BITMAP_OFFSET	\
	(offsetof(struct delfs_checkpoint, sit_nat_version_bitmap))
 ...
 * struct delfs_ophan_block_footer
 *
 * Do NOT use sizeof, use F2FS_BLKSIZE instead
 */
struct delfs_orphan_block {
...
static_assert(sizeof(struct delfs_extent) == 12, "");
...
#define F2FS_EXTRA_ISIZE_OFFSET				\
	offsetof(struct delfs_inode, i_extra_isize)
#define F2FS_TOTAL_EXTRA_ATTR_SIZE			\
	(offsetof(struct delfs_inode, i_extra_end) - F2FS_EXTRA_ISIZE_OFFSET)
 ...
 * On disk layout is
 * struct delfs_inode
 * struct delfs_inode_nids
 *
 * where the size of delfs_inode depends on block size.
 * Do NOT use sizeof
 */
struct delfs_inode {
	...
	struct delfs_extent i_ext __attribute__((packed));	/* caching a largest extent */
...
#define F2FS_INODE_NIDS(inode) ((struct delfs_inode_nids *)(&(inode)->i_addr[DEF_ADDRS_PER_INODE]))
#define F2FS_INODE_I_NID(inode, i) (F2FS_INODE_NIDS((inode))->i_nid[(i)])

static_assert(offsetof(struct delfs_inode, i_extra_end) -
	      offsetof(struct delfs_inode, i_extra_isize) == 36, "");
 ...
 * struct node_footer
 *
 * where the size of delfs_node depends on Block Size.
 * Do NOT use sizeof. Use F2FS_BLKSIZE instead.
 */
struct delfs_node {
	/* can be one of three types: inode, direct, and indirect types */
	union {
		struct delfs_inode i;
 ...
 * For NAT entries
 */
#define NAT_ENTRY_PER_BLOCK (F2FS_BLKSIZE / sizeof(struct delfs_nat_entry))
#define NAT_BLOCK_OFFSET(start_nid) (start_nid / NAT_ENTRY_PER_BLOCK)

#define DEFAULT_NAT_ENTRY_RATIO		20

struct delfs_nat_entry {
	__u8 version;		/* latest version of cached nat entry */
	__le32 ino;		/* inode number */
	__le32 block_addr;	/* block address */
} __attribute__((packed));

static_assert(sizeof(struct delfs_nat_entry) == 9, "");

struct delfs_nat_block {
	struct delfs_nat_entry entries[0]; /* NAT_ENTRY_PER_BLOCK */
};
...
#define SIT_ENTRY_PER_BLOCK (F2FS_BLKSIZE / sizeof(struct delfs_sit_entry))
...
/*
 * Note that delfs_sit_entry->vblocks has the following bit-field information.
...
struct delfs_sit_entry {
	__le16 vblocks;				/* reference above */
	__u8 valid_map[SIT_VBLOCK_MAP_SIZE];	/* bitmap for valid blocks */
	__le64 mtime;				/* segment age for cleaning */
} __attribute__((packed));

static_assert(sizeof(struct delfs_sit_entry) == 74, "");

/*
 * On disk layout is:
 * struct delfs_sit_entry entries[SIT_ENTRY_PER_BLOCK];
 */
struct delfs_sit_block {
	struct delfs_sit_entry entries[0];
};
...
static_assert(sizeof(struct delfs_summary) == 7, "");
...
/*
 * Reserved area should make size of delfs_extra_info equals to
...
struct delfs_journal {
	...
	union {
		...
		struct delfs_extra_info info;
	};
} __attribute__((packed));

/*
 * Block-sized summary block structure
 * Layout of delfs_summary block is
 * struct delfs_summary entries[ENTRIES_IN_SUM];
 ...
 *
 */
struct delfs_summary_block {
	struct delfs_summary entries[0];
};
#define F2FS_SUMMARY_BLOCK_JOURNAL(blk) ((struct delfs_journal *)(&(blk)->entries[ENTRIES_IN_SUM]))
...
static_assert(sizeof(struct delfs_dir_entry) == 11, "");

/*
 * Block-sized directory entry block
 * On disk structure:
 * struct delfs_dentry_block;
 * __u8 reserved[SIZE_OF_RESERVED];
 * struct delfs_dir_entry dentry[NR_DENTRY_IN_BLOCK];
...
#define F2FS_DENTRY_BLOCK_DENTRIES(blk) ((struct delfs_dir_entry *)\
...
union wrap_percore_sit_jnl {
	struct delfs_journal sit_jnl;
	char sit_bytes[F2FS_MAX_BLKSIZE];
};

union wrap_percore_nat_jnl {
	struct delfs_journal nat_jnl;
	char nat_bytes[F2FS_MAX_BLKSIZE];
};

struct delfs_configuration {
	...
	uint32_t **cur_segs; //taehwan code
	...
	/* compression support for sload.delfs */
	compress_config_t compress;

	block_t curseg_offset[NR_CURSEG_TYPE];
	block_t **curseg_offsets; //taehwan code
//	struct delfs_summary sum[NR_CURSEG_TYPE][MAX_CACHE_SUMS];
	struct delfs_summary sums[MAX_CORE_COUNT][NR_CURSEG_TYPE][MAX_CACHE_SUMS];
	union {
		struct delfs_journal sit_jnl;
		char sit_bytes[F2FS_MAX_BLKSIZE];
	};
	...
	union wrap_percore_sit_jnl percore_sit_jnl[MAX_CORE_COUNT];
	union wrap_percore_nat_jnl percore_nat_jnl[MAX_CORE_COUNT];
};
...
extern unsigned int addrs_per_page(struct delfs_inode *, bool);
extern unsigned int delfs_max_file_offset(struct delfs_inode *);
extern __u32 delfs_inode_chksum(struct delfs_node *);
extern __u32 delfs_checkpoint_chksum(struct delfs_checkpoint *);
extern int write_inode(struct delfs_node *, u64, enum rw_hint);

extern int get_bits_in_byte(unsigned char n);
extern int test_and_set_bit_le(u32, u8 *);
extern int test_and_clear_bit_le(u32, u8 *);
extern int test_bit_le(u32, const u8 *);
extern int delfs_test_bit(unsigned int, const char *);
extern int delfs_set_bit(unsigned int, char *);
extern int delfs_clear_bit(unsigned int, char *);
extern u64 find_next_bit_le(const u8 *, u64, u64);
extern u64 find_next_zero_bit_le(const u8 *, u64, u64);

extern uint32_t delfs_cal_crc32(uint32_t, void *, int);
extern int delfs_crc_valid(uint32_t blk_crc, void *buf, int len);

extern void delfs_init_configuration(void);
extern int delfs_devs_are_umounted(void);
extern int delfs_dev_is_writable(void);
extern int delfs_dev_is_umounted(char *);
extern int delfs_get_device_info(void);
extern int delfs_get_delfs_info(void);
extern unsigned int calc_extra_isize(void);
extern int get_device_info(int);
extern int delfs_init_sparse_file(void);
extern void delfs_release_sparse_resource(void);
extern int delfs_finalize_device(void);
...
#endif
extern enum rw_hint delfs_io_type_to_rw_hint(int);
...
delfs_hash_t delfs_dentry_hash(int, int, const unsigned char *, int);

static inline bool delfs_has_extra_isize(struct delfs_inode *inode)
{
	return (inode->i_inline & F2FS_EXTRA_ATTR);
}

static inline int __get_extra_isize(struct delfs_inode *inode)
{
	if (delfs_has_extra_isize(inode))
		return le16_to_cpu(inode->i_extra_isize) / sizeof(__le32);
	return 0;
}

extern struct delfs_configuration c;
static inline int get_inline_xattr_addrs(struct delfs_inode *inode)
{
...
extern int delfs_get_zoned_model(int);
extern int delfs_get_zone_blocks(int);
extern int delfs_report_zone(int, uint64_t, struct blk_zone *);
typedef int (report_zones_cb_t)(int i, void *, void *);
extern int delfs_report_zones(int, report_zones_cb_t *, void *);
extern int delfs_check_zones(int);
int delfs_reset_zone(int, void *);
extern int delfs_reset_zones(int);
int delfs_finish_zone(int i, void *blkzone);
extern uint32_t delfs_get_usable_segments(struct delfs_super_block *sb);
...
static inline uint32_t get_reserved(struct delfs_super_block *sb, double ovp)
{
	uint32_t usable_main_segs = delfs_get_usable_segments(sb);
...
static inline uint32_t overprovision_segment_buffer(struct delfs_super_block *sb)
{
	/* Give 6 current sections to avoid huge GC overheads. */
	return 6 * get_sb(segs_per_sec);
}

static inline double get_best_overprovision(struct delfs_super_block *sb)
{
...
static inline __le64 get_cp_crc(struct delfs_checkpoint *cp)
{
...
static inline int exist_qf_ino(struct delfs_super_block *sb)
{
	int i;
...
static inline int is_qf_ino(struct delfs_super_block *sb, nid_t ino)
{
	int i;
...
static inline void delfs_init_inode(struct delfs_super_block *sb,
		struct delfs_node *raw_node, nid_t ino, time_t mtime, mode_t mode)
{
 ...
 * NLS definitions
 */
struct delfs_nls_table {
	int version;
	const struct delfs_nls_ops *ops;
};

struct delfs_nls_ops {
	int (*casefold)(const struct delfs_nls_table *charset,
			const unsigned char *str, size_t len,
			unsigned char *dest, size_t dlen);
};

extern const struct delfs_nls_table *delfs_load_nls_table(int encoding);
#define F2FS_ENC_UTF8_12_0	1

extern int delfs_str2encoding(const char *string);
extern char *delfs_encoding2str(const int encoding);
extern int delfs_get_encoding_flags(int encoding);
extern int delfs_str2encoding_flags(char **param, __u16 *flags);
...
static inline void check_block_struct_sizes(void)
{
	...
	assert(offsetof(struct delfs_inode, i_extra_isize) + DEF_ADDRS_PER_INODE * sizeof(__le32)
		+ sizeof(struct delfs_inode_nids) + sizeof(struct node_footer) == F2FS_BLKSIZE);
	...
	assert((NAT_ENTRY_PER_BLOCK + 1) * sizeof(struct delfs_nat_entry) > F2FS_BLKSIZE);
	assert(NAT_ENTRY_PER_BLOCK * sizeof(struct delfs_nat_entry) <= F2FS_BLKSIZE);

	/* Check SIT Block Size */
	assert((SIT_ENTRY_PER_BLOCK + 1) * sizeof(struct delfs_sit_entry) > F2FS_BLKSIZE);
	assert(SIT_ENTRY_PER_BLOCK * sizeof(struct delfs_sit_entry) <= F2FS_BLKSIZE);

	/* Check NAT Journal Block Size */
	assert(sizeof(struct delfs_summary) * ENTRIES_IN_SUM
			+ offsetof(struct delfs_journal, nat_j)
	...
	assert(sizeof(struct delfs_summary) * ENTRIES_IN_SUM
			+ offsetof(struct delfs_journal, sit_j)
	...
	assert(sizeof(struct delfs_summary) * ENTRIES_IN_SUM + sizeof(__le64)
			+ offsetof(struct delfs_journal, info)
	...
	assert(sizeof(__u8) * (SIZE_OF_DENTRY_BITMAP + SIZE_OF_RESERVED)
			+ NR_DENTRY_IN_BLOCK * sizeof(struct delfs_dir_entry)
