...
struct delfs_super_block raw_sb;
struct delfs_super_block *sb = &raw_sb;
...
{
	//printf("=====[%s] start - seg_type=%d\n", __func__, seg_type);
	uint32_t next_seg = c.cur_seg[seg_type] + c.segs_per_zone;
//	printf("=====[%s] c.cur_seg[%d]=%d, c.segs_per_zone=%d\n", __func__,
//			seg_type, c.cur_seg[seg_type], c.segs_per_zone);
	uint64_t next_blkaddr = START_BLOCK(next_seg);
	int dev_num;

	dev_num = target_device_index(next_blkaddr);
//	printf("=====[%s] next_seg=%d, next_blkaddr=%d, dev_num=%d\n", __func__,
//			next_seg, next_blkaddr, dev_num);
	if (!device_is_aliased(dev_num)) { //o
//		printf("=====[%s] GET_SEGNO=%d, blk_addr=%d, main_blkaddr=%d, c.blks_per_seg=%d\n",
//				__func__, GET_SEGNO(next_blkaddr), next_blkaddr, get_sb(main_blkaddr),
//				c.blks_per_seg);
	...
	for (cpu = 0; cpu < num_cpus; cpu++) {
		for (i = 0; i < NR_CURSEG_TYPE; i++) {
			for (j = i + 1; j < NR_CURSEG_TYPE; j++) {
				if (c.cur_segs[cpu][i] == c.cur_segs[cpu][j]) {
					reorder = 1;
					break;
				}
			}
		...
		for (i = 1; i < NR_CURSEG_TYPE; i++) {
			//c.cur_segs[cpu][i] = next_zone(i - 1);
		}
	}
}

static int delfs_prepare_super_block(void)
{
	...
	if (c.large_nat_bitmap) {
		/* use cp_payload if free space of delfs_checkpoint is not enough */
		...
		if (max_sit_bitmap_size > MAX_SIT_BITMAP_SIZE_IN_CKPT) {
			...
			max_nat_bitmap_size = MAX_BITMAP_SIZE_IN_CKPT -
							max_sit_bitmap_size;
			set_sb(cp_payload, 0);
			//set_sb(cp_payload, 2); //taehwan code
		}
...
//
//	int cp_payload_blk_count = ((num_cpus * (4 + 2 + 4 + 2) * 8) + 4095) / 4096;
//	set_sb(cp_payload, cp_payload_blk_count);
//	/* taehwan code */
...
//	}
	...
	for (i = 0; i < num_cpus; i++) {
		int percore_total_zones = total_zones / num_cpus;
		int percore_seg0_addr = percore_total_zones * i;
		c.cur_segs[i][CURSEG_HOT_NODE] = percore_seg0_addr;
		c.cur_segs[i][CURSEG_WARM_NODE] = percore_seg0_addr+1;
		c.cur_segs[i][CURSEG_COLD_NODE] = percore_seg0_addr+2;
		c.cur_segs[i][CURSEG_HOT_DATA] = percore_seg0_addr+3;
		c.cur_segs[i][CURSEG_COLD_DATA] = percore_seg0_addr + 
							max(percore_total_zones >> 2, 4);
		c.cur_segs[i][CURSEG_WARM_DATA] = percore_seg0_addr + 
							max(percore_total_zones >> 1, 5);
	...
	if (c.feature & F2FS_FEATURE_SB_CHKSUM) {
		...
		set_sb(crc, delfs_cal_crc32(F2FS_SUPER_MAGIC, sb,
						SB_CHKSUM_OFFSET));
...
static int delfs_init_sit_area(void)
{
...
static int delfs_init_nat_area(void)
{
	uint32_t blk_size, seg_size;
	uint32_t index = 0;
...
static int delfs_write_check_point_pack(void)
{
	struct delfs_summary_block *sum;
	...
	unsigned int i, j, k, cpu;
	char *cp_payload = NULL;
	char *sum_compact, *sum_compact_p;
	struct delfs_summary *sum_entry;
	unsigned short vblocks = 0;
	uint32_t used_segments = c.aliased_segments;
	int ret = -1;
	long num_cpus = sysconf(_SC_NPROCESSORS_ONLN);

	//cp = calloc(F2FS_BLKSIZE, 1);
	cp = calloc(F2FS_BLKSIZE, 4);
	if (cp == NULL) {
		MSG(1, "\tError: Calloc failed for delfs_checkpoint!!!\n");
		return ret;
	}

//	for (i = 0; i < MAX_CORE_COUNT; i++) {
//		for (j = 0; j < MAX_ACTIVE_NODE_LOGS; j++) {
//			cp->cur_nodes_segno[i][j] = 0;
//			cp->cur_nodes_blkoff[i][j] = 0;
//			cp->cur_datas_segno[i][j] = 0;
//			cp->cur_datas_blkoff[i][j] = 0;
//		} 
//	}
...
//	}
	...
	/* taehwan code */
	uint64_t total_meta_segments = get_sb(segment_count_ckpt) + 
		get_sb(segment_count_sit) + 
		get_sb(segment_count_nat) +
		get_sb(segment_count_ssa);
	uint64_t total_meta_zones =  ZONE_ALIGN(total_meta_segments * 
							c.blks_per_seg);
	uint32_t total_zones = get_sb(segment_count) / (c.segs_per_zone) - 
								total_meta_zones;
	if (num_cpus < 1) num_cpus = 1;

	for (cpu = 0; cpu < num_cpus; cpu++) {
		int percore_total_zones = total_zones / num_cpus;
		set_cp(cur_nodes_segno[cpu][0], 
				c.cur_segs[cpu][CURSEG_HOT_NODE]);
		set_cp(cur_nodes_segno[cpu][1], 
				c.cur_segs[cpu][CURSEG_WARM_NODE]);
		set_cp(cur_nodes_segno[cpu][2], 
				c.cur_segs[cpu][CURSEG_COLD_NODE]);
		set_cp(cur_datas_segno[cpu][0], 
				c.cur_segs[cpu][CURSEG_HOT_DATA]);
		set_cp(cur_datas_segno[cpu][1], 
				c.cur_segs[cpu][CURSEG_WARM_DATA]);
		set_cp(cur_datas_segno[cpu][2], 
				c.cur_segs[cpu][CURSEG_COLD_DATA]);
		for (i = 3; i < MAX_ACTIVE_NODE_LOGS; i++) {
			set_cp(cur_nodes_segno[cpu][i], 0xffffffff);
			set_cp(cur_datas_segno[cpu][i], 0xffffffff);
		}
	}
...
//	set_cp(cur_data_blkoff[2], c.curseg_offset[CURSEG_COLD_DATA]);
	...
	/* taehwan code */
	for (cpu = 0; cpu < num_cpus; cpu++) {
		set_cp(cur_nodes_blkoff[cpu][0], c.curseg_offsets[cpu][CURSEG_HOT_NODE]);
		set_cp(cur_nodes_blkoff[cpu][2], c.curseg_offsets[cpu][CURSEG_COLD_NODE]);
		set_cp(cur_datas_blkoff[cpu][0], c.curseg_offsets[cpu][CURSEG_HOT_DATA]);
		set_cp(cur_datas_blkoff[cpu][2], c.curseg_offsets[cpu][CURSEG_COLD_DATA]);
	}

	/* taehwan code */

	set_cp(valid_block_count, c.curseg_offsets[0][CURSEG_HOT_NODE] +
			c.curseg_offsets[0][CURSEG_HOT_DATA] +
			c.curseg_offsets[0][CURSEG_COLD_NODE] +
			c.curseg_offsets[0][CURSEG_COLD_DATA] +
	 ...
	 */
	set_cp(overprov_segment_count, (delfs_get_usable_segments(sb) -
	...
	if (delfs_get_usable_segments(sb) <= get_cp(overprov_segment_count)) {
	...
	set_cp(user_block_count, (delfs_get_usable_segments(sb) -
			get_cp(overprov_segment_count)) * c.blks_per_seg);
	set_cp(free_segment_count, delfs_get_usable_segments(sb) -
			used_segments);

	/* cp page (2), data summaries (1), node summaries (3) */
	/* percore - cp page (8), data summaries (128), node summaries (128 * 3) */
	//set_cp(cp_pack_total_block_count, 6 + get_sb(cp_payload)); //taehwan code
	set_cp(cp_pack_total_block_count, 8 + num_cpus + (num_cpus * 3) + get_sb(cp_payload));
	flags = CP_UMOUNT_FLAG | CP_COMPACT_SUM_FLAG;
	if (get_cp(cp_pack_total_block_count) <=
			(1 << get_sb(log_blocks_per_seg) * 2) - nat_bits_blocks) {
...
//			c.curseg_offset[CURSEG_COLD_NODE]);
	...
	/* taehwan code */
	set_cp(cp_pack_start_sum, 1 + 3 + get_sb(cp_payload));
	set_cp(valid_node_count, c.curseg_offsets[0][CURSEG_HOT_NODE] +
			c.curseg_offsets[0][CURSEG_COLD_NODE]);
	set_cp(valid_inode_count, c.curseg_offsets[0][CURSEG_HOT_NODE] +
			c.curseg_offsets[0][CURSEG_COLD_NODE]);
	/* taehwan code */
...
//	set_cp(compact_page_count, 1);
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	if (cp_dev_write_block(cp, cp_seg_blk, WRITE_LIFE_NONE)) {
		MSG(1, "\tError: While writing the cp to disk!!!\n");
		goto free_cp_payload;
	}
	cp_seg_blk+=3; //taehwan code
	...
	for (i = 0; i < get_sb(cp_payload); i++) {
		...
		if (dev_fill_block(cp_payload, cp_seg_blk, WRITE_LIFE_LONG)) {
...
for (cpu = 0; cpu < num_cpus; cpu++) {
	if (cpu < 6)
	memset(sum_compact, 0, F2FS_BLKSIZE);
	sum_compact_p =sum_compact;
/* taehwan code */

	/* nat_sjournal */
	//journal = &c.nat_jnl;
	journal = &c.percore_nat_jnl[cpu].nat_jnl;
	...
	/* sit_journal */
	//journal = &c.sit_jnl;
	journal = &c.percore_sit_jnl[cpu].sit_jnl;

	if (c.feature & F2FS_FEATURE_RO) { //x
...
//		journal->n_sits = cpu_to_le16(6);
		...
		for (i = CURSEG_HOT_DATA; i < NR_CURSEG_TYPE; i++) {
			if (i < NR_CURSEG_DATA_TYPE) {
				journal->sit_j.entries[i].segno =
					cp->cur_datas_segno[cpu][i];

			} else {
				journal->sit_j.entries[i].segno =
					cp->cur_nodes_segno[cpu][i - NR_CURSEG_DATA_TYPE];
			}
		...
		/* taehwan debug code */
//		for (i = 0; i < 6; i++) {
//		if (cpu < 5 && journal->sit_j.entries[i].segno < 100) {
//			int tmp_i, tmp_j;
//			for (tmp_i = 0; tmp_i < 64; tmp_i++) {
//				__u8 tmp = journal->sit_j.entries[i].se.valid_map[tmp_i];
//				for (tmp_j = 0; tmp_j < 8; tmp_j++) {
//					if (tmp & (1UL) << 7-tmp_j)
//						printf("[%s] 2 segno=%d, vblocks=%d, valid_map=%d, offset=%d\n",
//								__func__, journal->sit_j.entries[i].segno,
//								journal->sit_j.entries[i].se.vblocks,
//								tmp_i, tmp_j);
//				}
//			}
...
//			int percore_total_zones = total_zones / num_cpus;
//			int percore_seg0_addr = percore_total_zones * j;
//			for (i = CURSEG_HOT_DATA; i < NR_CURSEG_TYPE; i++) {
//				if (i < NR_CURSEG_DATA_TYPE) {
//					journal->sit_j.entries[n_cnt].segno =
//						percore_seg0_addr + cp->cur_datas_segno[j][i];
//				} else {
//					journal->sit_j.entries[n_cnt].segno =
//						percore_seg0_addr + cp->cur_nodes_segno[j][i - NR_CURSEG_DATA_TYPE];
//				}
//
//				vblocks =
//					le16_to_cpu(journal->sit_j.entries[n_cnt].se.vblocks);
//				journal->sit_j.entries[n_cnt].se.vblocks =
//							cpu_to_le16(vblocks | (i << 10));
//				n_cnt++;
//			}
//		}
//		journal->n_sits = cpu_to_le16(6 * num_cpus);
//
//		/* taehwan code */
	}
	...
	sum_entry = (struct delfs_summary *)sum_compact_p;
	memcpy(sum_entry, c.sums[cpu][CURSEG_HOT_DATA],
			sizeof(struct delfs_summary) * MAX_CACHE_SUMS);
//	for (i = 0; i < 8; i++) {
//		printf("=====[%s] c.sums[cpu=%d][CURSEG_HOT_DATA][i=%d].nid=%d\n", __func__,
//				cpu, i, c.sums[cpu][CURSEG_HOT_DATA][i].nid);
//	}

//	printf("=====[%s] MAX_CACHE_SUMS=%d, sizeof(struct delfs_summary)=%d\n", 
//			__func__, MAX_CACHE_SUMS,
//			sizeof(struct delfs_summary)); //CACHE_SUM=8, delfs_summary=7
	...
	cp_seg_blk++;
//	printf("=====[%s] cpu=%d, sum_com_blk=%d\n", __func__, cpu, cp_seg_blk);
	DBG(1, "\tWriting Segment summary for HOT/WARM/COLD_DATA, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
//	printf("[%s] 3 compacted cp_seg_blk=%d\n", __func__, cp_seg_blk);
	...
	memcpy(sum->entries, c.sums[cpu][CURSEG_HOT_NODE],
			sizeof(struct delfs_summary) * MAX_CACHE_SUMS);
//	for (i = 0; i < 8; i++) {
//		printf("=====[%s] c.sums[cpu=%d][CURSEG_HOT_NODE][i=%d].nid=%d\n", __func__,
//				cpu, i, c.sums[cpu][CURSEG_HOT_NODE][i].nid);
//	}

	cp_seg_blk++;
//	printf("=====[%s] 5 cp_seg_blk=%d\n", __func__, cp_seg_blk);
	DBG(1, "\tWriting Segment summary for HOT_NODE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
//	printf("[%s] 3 hot node cp_seg_blk=%d\n", __func__, cp_seg_blk);
...
//	printf("[%s] 4 warm node cp_seg_blk=%d\n", __func__, cp_seg_blk);
	...
	memset(sum, 0, F2FS_BLKSIZE);
	SET_SUM_TYPE(sum, SUM_TYPE_NODE);
	memcpy(sum->entries, c.sums[cpu][CURSEG_COLD_NODE],
			sizeof(struct delfs_summary) * MAX_CACHE_SUMS);

	cp_seg_blk++;
//	printf("=====[%s] 7 cp_seg_blk=%d\n", __func__, cp_seg_blk);
	DBG(1, "\tWriting Segment summary for COLD_NODE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
//	printf("[%s] 5 cold node cp_seg_blk=%d\n", __func__, cp_seg_blk);
	if (dev_write_block(sum, cp_seg_blk, WRITE_LIFE_NONE)) {
		MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
		goto free_cp_payload;
	}
...
/* taehwan code */
	...
	if (flags & CP_NAT_BITS_FLAG) {
		...
		*(__le64 *)nat_bits = get_cp_crc(cp);
		...
		//cp_seg_blk = get_sb(segment0_blkaddr) + (1 << //taehwan
		cp_seg_blk = get_sb(segment0_blkaddr) + (2 <<
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	cp_seg_blk = get_sb(segment0_blkaddr) + (c.blks_per_seg * 2);
...
free_sum:
	free(sum);
/* taehwan code */
//free_cur_datas_blkoff:
//	free(cp->cur_datas_blkoff);
//free_cur_datas_segno:
//	free(cp->cur_datas_segno);
//free_cur_nodes_blkoff:
//	free(cp->cur_nodes_blkoff);
//free_cur_nodes_segno:
//	free(cp->cur_nodes_segno);
/* taehwan code */
free_cp:
	free(cp);
	return ret;
...
static int delfs_write_super_block(void)
{
	int index;
	uint8_t *zero_buff;
...
//			printf("[%s] NR_CURSEG_TYPE=%d\n", __func__, j);
//			for (k = 0; k < MAX_CACHE_SUMS; k++) {
//				printf("%d ", c.sums[i][j][k]);
//			}
//			printf("\n");
//		}
//	}
...
#ifndef WITH_ANDROID
static int delfs_discard_obsolete_dnode(void)
{
	struct delfs_node *raw_node;
	...
	offset += c.cur_segs[0][CURSEG_WARM_NODE] * c.blks_per_seg;

	last_inode_pos = start_inode_pos +
		c.cur_segs[0][CURSEG_HOT_NODE] * c.blks_per_seg +
		c.curseg_offsets[0][CURSEG_COLD_NODE] - 1;
	...
	do {
		...
		if (dev_write_block(raw_node, offset,
				    delfs_io_type_to_rw_hint(CURSEG_WARM_NODE))) {
...
//
//	c.curseg_offset[curseg_type]++;
//	printf("=====[%s]: 1 main_blkaddr=%d, c.blks_per_seg=%d\n", __func__,
//			get_sb(main_blkaddr), c.blks_per_seg);
//	printf("=====[%s]: blkaddr=%d, curseg=%d, curseg_offset[%d]=%d\n", __func__,
//			blkaddr, c.cur_seg[curseg_type], curseg_type, 
//			c.curseg_offset[curseg_type]);
	...
	blkaddr = get_sb(main_blkaddr) + 
			c.cur_segs[0][curseg_type] * c.blks_per_seg +
			c.curseg_offsets[0][curseg_type];
...
//static block_t sf_alloc_next_free_block(int curseg_type)
//{
//	block_t blkaddr;
//
//	blkaddr = get_sb(main_blkaddr) +
...
{
	//struct delfs_journal *sit_jnl = &c.sit_jnl;
	struct delfs_journal *sit_jnl = &c.percore_sit_jnl[0].sit_jnl;
	...
	delfs_set_bit(c.curseg_offsets[0][curseg_type] - 1,
...
{
	//struct delfs_journal *nat_jnl = &c.nat_jnl;
	struct delfs_journal *nat_jnl = &c.percore_nat_jnl[0].nat_jnl;
					...
					unsigned short ofs_in_node)
{
	...
	unsigned int curofs = c.curseg_offsets[0][curseg_type] - 1;

	assert(curofs < MAX_CACHE_SUMS);

	sum = c.sums[0][curseg_type] + curofs;
	sum->nid = cpu_to_le32(nid);
	sum->ofs_in_node = cpu_to_le16(ofs_in_node);
}

static void add_dentry(struct delfs_dentry_block *dent_blk, unsigned int *didx,
		const char *name, uint32_t ino, u8 type)
{
	...
	if (name[0] == '.' && (len == 1 || (len == 2 && name[1] == '.')))
		...
		hash = delfs_dentry_hash(0, 0, (unsigned char *)name, len);
...
static block_t delfs_add_default_dentry_root(void)
{
	struct delfs_dentry_block *dent_blk = NULL;
	...
	if (dev_write_block(dent_blk, data_blkaddr,
			    delfs_io_type_to_rw_hint(CURSEG_HOT_DATA))) {
...
static int delfs_write_root_inode(void)
{
	struct delfs_node *raw_node = NULL;
	block_t data_blkaddr;
	...
	delfs_init_inode(sb, raw_node, le32_to_cpu(sb->root_ino),
						mkfs_time, 0x41ed);
	...
	data_blkaddr = delfs_add_default_dentry_root();
	if (data_blkaddr == 0) {
	...
	//node_blkaddr = sf_alloc_next_free_block(CURSEG_HOT_NODE);
	...
	if (write_inode(raw_node, node_blkaddr,
			delfs_io_type_to_rw_hint(CURSEG_HOT_NODE)) < 0) {
...
static int delfs_write_default_quota(int qtype, __le32 raw_id)
{
...
static int delfs_write_qf_inode(int qtype)
{
	struct delfs_node *raw_node = NULL;
	block_t data_blkaddr;
	block_t node_blkaddr;
	...
	/* write quota blocks */
	data_blkaddr = delfs_write_default_quota(qtype, raw_id);
	if (data_blkaddr == 0) {
		free(raw_node);
		return -1;
	}
...
static int delfs_update_nat_default(void)
{
	struct delfs_nat_block *nat_blk = NULL;
...
static block_t delfs_add_default_dentry_lpf(void)
{
	struct delfs_dentry_block *dent_blk;
	block_t data_blkaddr;
	unsigned int didx = 0;
...
static int delfs_write_lpf_inode(void)
{
	struct delfs_node *raw_node;
	block_t data_blkaddr;
	block_t node_blkaddr;
	...
	delfs_init_inode(sb, raw_node, c.lpf_ino, mkfs_time, 0x41c0);
	...
	data_blkaddr = delfs_add_default_dentry_lpf();
	if (data_blkaddr == 0) {
...
static void allocate_blocks_for_aliased_device(struct delfs_node *raw_node,
		unsigned int dev_num)
{
	...
	uint32_t segno;
	uint64_t blkcnt;
	struct delfs_sit_block *sit_blk = calloc(F2FS_BLKSIZE, 1);
	...
	for (segno = start_segno; segno < end_segno; segno++) {
		...
		ASSERT(dev_write_block(sit_blk, sit_blk_addr,
			delfs_io_type_to_rw_hint(CURSEG_COLD_DATA)) >= 0);
	}
...
static int delfs_write_alias_inodes(void)
{
	struct delfs_node *raw_node;
	block_t node_blkaddr;
	int err = 0;
	...
	for (i = 1; i < c.ndevs; i++) {
		...
		dev_off++;
		delfs_init_inode(sb, raw_node, ino, mkfs_time, 0x81c0);
		...
		DBG(1, "\tWriting aliased device inode (cold node), "
				"offset 0x%x\n", node_blkaddr);
		if (write_inode(raw_node, node_blkaddr,
			    delfs_io_type_to_rw_hint(CURSEG_COLD_NODE)) < 0) {
...
static int delfs_create_root_dir(void)
{
	enum quota_type qtype;
	int err = 0;

	err = delfs_write_root_inode();
	if (err < 0) {
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++)  {
		if (!((1 << qtype) & c.quota_bits))
			continue;
		err = delfs_write_qf_inode(qtype);
		if (err < 0) {
...
#ifndef WITH_ANDROID
	err = delfs_discard_obsolete_dnode();
	if (err < 0) {
