...
extern struct sparse_file *delfs_sparse_file;
#endif
...
{
	MSG(0, "\nUsage: mkfs.delfs [options] device [sectors]\n");
	MSG(0, "[options]:\n");
...
static void delfs_show_info()
{
	MSG(0, "\n    F2FS-tools: mkfs.delfs Ver: %s (%s)\n\n",
				F2FS_TOOLS_VERSION,
				F2FS_TOOLS_DATE);
	...
	if (c.feature & F2FS_FEATURE_CASEFOLD)
		MSG(0, "Info: Enable %s with casefolding\n",
					delfs_encoding2str(c.s_encoding));
...
static void delfs_parse_options(int argc, char *argv[])
{
...
#ifdef HAVE_LIBBLKID
static int delfs_dev_is_overwrite(const char *device)
{
	const char	*type;
	blkid_probe	pr = NULL;
	int		ret = -1;
...
static int delfs_check_overwrite(void)
{
	int i;

	for (i = 0; i < c.ndevs; i++)
		if (delfs_dev_is_overwrite((char *)c.devices[i].path))
			return -1;
	return 0;
}
...
/* taehwan code */
void free_delfs_configuration(void)
{
	int i;
...
int main(int argc, char *argv[])
{
	...
	if (delfs_get_device_info() < 0)
		return -1;

	if (delfs_check_overwrite()) {
		char *zero_buf = NULL;
		int i;
	...
	if (delfs_format_device() < 0)
		goto err_format;

	if (delfs_finalize_device() < 0)
		goto err_format;

	/* taehwan code */
	free_delfs_configuration();
	/* taehwan code */
