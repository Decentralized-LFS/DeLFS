/*
 * delfscrypt.c - delfs encryption management utility
 *
...
struct delfs_encryption_key {
        __u32 mode;
        char raw[F2FS_MAX_KEY_SIZE];
        __u32 size;
};

static_assert(sizeof(struct delfs_encryption_key) == 72, "");

int options;

extern void delfs_sha512(const unsigned char *in, unsigned long in_size,
						unsigned char *out);
...
#define F2FS_KEY_DESC_PREFIX "delfs:"
			  ...
			  unsigned char derived_key[F2FS_MAX_KEY_SIZE])
{
	...
	for (x = 0; x < count; ++x) {
		if (x == 0) {
			delfs_sha512(saltbuf, actual_saltbuf_len, tempbuf);
		} else {
			/*
			 * buf: [previous hash || passphrase]
			 */
			memcpy(buf, tempbuf, SHA512_LENGTH);
			delfs_sha512(buf, actual_buf_len, tempbuf);
		}
...
static int get_keyring_id(const char *keyring)
{
	 ...
	 * garbage collected as soon as delfscrypt exits.
	 *
...
static void generate_key_ref_str(struct salt *salt)
{
	...
	delfs_sha512(salt->key, F2FS_MAX_KEY_SIZE, key_ref1);
	delfs_sha512(key_ref1, SHA512_LENGTH, key_ref2);
...
static void insert_key_into_keyring(const char *keyring, struct salt *salt)
{
	...
	struct delfs_encryption_key key;
...
#define add_key_help \
"delfscrypt add_key -S salt [ -k keyring ] [-v] [-q] [ path ... ]\n\n" \
"Prompts the user for a passphrase and inserts it into the specified\n" \
"keyring.  If no keyring is specified, delfscrypt will use the session\n" \
"keyring if it exists or the user session keyring if it does not.\n\n" \
"If one or more directory paths are specified, delfscrypt will try to\n" \
...
"delfscrypt set_policy policy path ... \n\n" \
"Sets the policy for the directories specified on the command line.\n" \
"All directories must be empty to set the policy; if the directory\n" \
"already has a policy established, delfscrypt will validate that it the\n" \
