/*
 * delfs_io.c - delfs ioctl utility
 *
 * Author: Jaegeuk Kim <jaegeuk@kernel.org>
 *
 * Copied portion of the code from ../delfscrypt.c
 */
...
#define fadvise_help						\
"delfs_io fadvise [advice] [offset] [length] [file]\n\n"		\
...
#define pinfile_help						\
"delfs_io pinfile [get|set|unset] [file]\n\n"			\
...
"delfs_io fallocate [-c] [-i] [-p] [-z] [keep_size] [offset] [length] [file]\n\n"	\
...
#define erase_help				\
"delfs_io erase [block_device_path]\n\n"		\
...
#define write_help					\
"delfs_io write [chunk_size in 4kb] [offset in chunk_size] [count] [pattern] [IO] [file_path] {delay}\n\n"	\
...
"delfs_io write_advice [chunk_size in 4kb] [offset in chunk_size] [count] [pattern] [IO] [advise] [file_path] {delay}\n\n"	\
"Write given patten data in file_path\n"		\
"pattern can be\n"					\
"  zero          : zeros\n"				\
"  inc_num       : incrementing numbers\n"		\
...
#define read_help					\
"delfs_io read [chunk_size in 4kb] [offset in chunk_size] [count] [IO] [advice] [print_nbytes] [file_path]\n\n"	\
...
"delfs_io randread [chunk_size in 4kb] [count] [IO] [advise] [file_path]\n\n"	\
...
#define fiemap_help					\
"delfs_io fiemap [offset in 4kb] [count in 4kb] [file_path]\n\n"\
...
"delfs_io gc_urgent $dev [start/end/run] [time in sec]\n\n"\
" - delfs_io gc_urgent sda21 start\n"		\
" - delfs_io gc_urgent sda21 end\n"		\
" - delfs_io gc_urgent sda21 run 10\n"		\
...
static void do_gc_urgent(int argc, char **argv, const struct cmd_desc *cmd)
{
	...
	if (argc == 3 && !strcmp(argv[2], "start")) {
		...
		sprintf(command, "echo %d > %s/%s/gc_urgent", 1, "/sys/fs/delfs/", argv[1]);
		if (system(command))
			exit(1);
	} else if (argc == 3 && !strcmp(argv[2], "end")) {
		printf("gc_urgent: end on %s\n", argv[1]);
		sprintf(command, "echo %d > %s/%s/gc_urgent", 0, "/sys/fs/delfs/", argv[1]);
		if (system(command))
			exit(1);
...
"delfs_io defrag_file [start] [length] [file_path]\n\n"		\
...
#define copy_help							\
"delfs_io copy [-d] [-m] [-s] src_path dst_path\n\n"			\
...
#define get_cblocks_help "delfs_io get_cblocks [file]\n\n"
...
#define release_cblocks_help "delfs_io release_cblocks [file]\n\n"
...
#define reserve_cblocks_help "delfs_io reserve_cblocks [file]\n\n"
...
{
	struct delfs_comp_option option;
	int ret, fd;
...
"delfs_io set_coption [algorithm] [log_cluster_size] [file_path]\n\n"	\
"  algorithm        : compression algorithm (0:lzo, 1: lz4, 2:zstd, 3:lzorle)\n"	\
"  log_cluster_size : compression cluster log size (2 <= log_size <= 8)\n"
...
#define decompress_help "delfs_io decompress [file_path]\n\n"
...
#define compress_help "delfs_io compress [file_path]\n\n"
...
"delfs_io filename_encrypt_mode [file or directory path]\n\n"		\
...
#define rename_help							\
"delfs_io rename [src_path] [target_path] [fsync_after_rename]\n\n"	\
"e.g., delfs_io rename source dest 1\n"					\
"      1. open(source)\n"						\
...
#define gc_help "delfs_io gc sync_mode [file_path]\n\n"
...
#define checkpoint_help "delfs_io checkpoint [file_path]\n\n"
...
#define precache_extents_help "delfs_io precache_extents [file_path]\n\n"
...
"delfs_io move_range [src_path] [dst_path] [src_start] [dst_start] "	\
"[length]\n\n"								\
"  src_path  : path to source file\n"					\
"  dst_path  : path to destination file\n"				\
...
#define gc_range_help "delfs_io gc_range [sync_mode] [start] [length] [file_path]\n\n"\
...
#define listxattr_help "delfs_io listxattr [file_path]\n\n"
...
#define setxattr_help "delfs_io setxattr [name] [value] [file_path]\n\n"
...
#define removexattr_help "delfs_io removexattr [name] [file_path]\n\n"
...
#define lseek_help					\
"delfs_io lseek [whence] [offset] [file_path]\n\n"	\
...
#define get_advise_help "delfs_io get_advise [file_path]\n\n"
